...
auto_cut_bot explicitly supports Windows. Key differences to keep in mind:
...
Agent system prompts and scenario-specific instructions live in `auto_cut_bot/templates/` as Jinja2 markdown files (`identity.md`, `platform_policy.md`, `HEARTBEAT.md`, `SOUL.md`, etc.). Changing these files alters agent behavior as directly as changing Python code. They are loaded by `utils/prompt_templates.py`.
...
Built-in skills live in `auto_cut_bot/skills/` (markdown + YAML frontmatter format). Agent capabilities that are "know-how" rather than code should be added as skills, not hardcoded into the agent loop. External skills can be published to and installed from ClawHub.
