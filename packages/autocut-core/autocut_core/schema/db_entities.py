...
class Shot(BaseModel):
    ...
    precise_start: float | None = None
    precise_end: float | None = None
    global_rank: int | None = None
    rank_score: float | None = None
    rank_criteria: dict[str, Any] | None = None
...
# ── 11. global_context — 全剧上下文缓存 ────────────────────────────────────


class GlobalContext(BaseModel):
    """全剧上下文缓存，VLM 上下文注入的全局数据源。

    对应: autocut.global_context
    数据来源: VLM 预处理 + API 数据合并
    """

    book_id: str = Field(..., description="剧的唯一标识，主键")
    synopsis: str | None = Field(None, description="全剧剧情概要")
    themes: list[str] = Field(default_factory=list, description="主题标签")
    relationships: dict[str, Any] | None = Field(None, description="角色关系 JSONB")
    source: str | None = Field(None, description="数据来源: vlm/api/manual")
    fetched_at: datetime | None = Field(None, description="最后一次获取时间")


# ── 12. vlm_confidence_log — VLM 置信度审计日志 ────────────────────────────


class VlmConfidenceLog(BaseModel):
    """VLM 窗口分析的置信度审计日志。

    对应: autocut.vlm_confidence_log
    每个窗口一条记录，追踪 VLM 输出的置信度分布。
    """

    id: int | None = Field(None, description="自增主键 (SERIAL)")
    book_id: str = Field(..., description="所属剧 ID")
    window_id: str = Field(..., description="窗口唯一标识")
    total_dialogue: int | None = Field(None, description="窗口内总对白数")
    high_conf: int | None = Field(None, description="高置信度条目数")
    low_conf: int | None = Field(None, description="低置信度条目数")
    characters_seen: list[str] = Field(default_factory=list, description="窗口中出现的角色")
    has_hard_subtitles: bool = Field(default=False, description="是否包含硬字幕")
    enrichment_triggered: bool = Field(
        default=False, description="是否触发了上下文补充"
    )
    created_at: datetime | None = None


# ── 13. highlight_skill_evolution — 高光技能进化 ────────────────────────────


class HighlightSkillEvolution(BaseModel):
    """高光识别技能进化日志。

    对应: autocut.highlight_skill_evolution
    记录每次 VLM 高光识别与 API 高光的对比，用于技能迭代。
    """

    id: int | None = Field(None, description="自增主键 (SERIAL)")
    skill_version: str = Field(..., description="技能版本号")
    window_id: str = Field(..., description="窗口唯一标识")
    api_highlight: dict[str, Any] = Field(
        default_factory=dict, description="API 返回的高光 JSONB"
    )
    vlm_miss_reason: str | None = Field(None, description="VLM 漏检原因")
    skill_update: str | None = Field(None, description="技能更新内容")
    created_at: datetime | None = None
...
# 跨表联合类型
Entity = Book | Subject | Relationship | Episode | Subtitle | SpeakerMapping | SubjectEpisode | Shot | Scene | Boundary | GlobalContext | VlmConfidenceLog | HighlightSkillEvolution
...
TABLE_NAMES: dict[str, type[BaseModel]] = {
    ...
    "vlm_confidence_log": VlmConfidenceLog,
    "highlight_skill_evolution": HighlightSkillEvolution,
}