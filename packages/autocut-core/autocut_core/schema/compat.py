...
FACT_ID = {"type": "string", "pattern": r"^fact-[a-z0-9-]{2,80}$"}
...
SERIES_BIBLE_SCHEMA = obj(
    {
        ...
        "genre_evidence_event_ids": arr(STR),
        "genre_review_status": {"type": "string"},
        "golden_case_ids": arr(STR),
    },
    required=[
        "schema_version", "metadata", "series_summary", "characters",
        "main_characters", "entity_importance", "relationships", "facts",
        "story_threads", "thread_beats", "open_questions",
        "unresolved_identity_conflicts", "coverage",
    ],
)
...
SCHEMAS = {
    "vlm_analysis": WINDOW_ANALYSIS_SCHEMA,
    "window_analysis": WINDOW_ANALYSIS_SCHEMA,  # backward compat
...
TASK_PROMPTS = {
    ...
    "window_analysis": (  # backward compat — same as vlm_analysis
        "直接理解提供的连续视频窗。完整提取剧情节拍、对白与屏幕文字、视觉事件、"
        "人物、因果、时间模式、窗口首尾状态以及 Highlight/Hook 候选。"
        "Hook 必须完整抛出问题但不能给出答案。"
        "story_beats、dialogue_and_text 和 visual_events 应保留完整句子、动作与反应；"
