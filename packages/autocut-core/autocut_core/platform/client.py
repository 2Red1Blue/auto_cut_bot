...
class PlatformAPIClient:
    ...
    def fetch_book_metadata(self, book_id: str) -> dict[str, Any]:
        ...
        try:
            result = self._request(
                ...
                json={"bookIds": [book_id], "language": "ENGLISH"},
            )
            # API 返回 {"data": {"bookId": {...}}} 格式
    ...
    def fetch_episodes(self, book_id: str) -> dict[str, Any]:
        """Fetch episode details from the batch-episodes-info endpoint.

        API: POST /assets/tmp/batch-episodes-info
        Returns a dict with keys:
          - book_name (str): book name from API
          - episodes (list): list of episode dicts
        ...
        try:
            ...
            # API 返回 {"data": {"bookId": {"episodes": [...]}}} 格式
            if isinstance(result, dict):
                data = result.get("data", result)
                if isinstance(data, dict) and book_id in data:
                    book_data = data[book_id]
                    episodes = book_data.get("episodes", [])
                    book_name = book_data.get("bookName") or book_data.get("book_name", "")
                    if isinstance(episodes, list):
                        return {"book_name": book_name, "episodes": episodes}
                # Some APIs wrap episodes in a dict key
                episodes = result.get("episodes", result.get("data", []))
                if isinstance(episodes, list):
                    return {"book_name": "", "episodes": episodes}
                # Fallback
                return {"book_name": "", "episodes": [result]}
            if isinstance(result, list):
                return {"book_name": "", "episodes": result}
            return {"book_name": "", "episodes": []}
        except Exception as exc:
            logger.warning(
