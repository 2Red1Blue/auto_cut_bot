...
def _render_bible(value: dict[str, Any]) -> str:
    ...
    relationship_by_id = {
        rel["id"]: rel for rel in relationships if isinstance(rel, dict)
    }
    story_threads = value.get("story_threads", [])
    thread_char_sets: dict[str, set[str]] = {}
    for ft in story_threads:
    ...
    for char_id in main_ids:
        ...
        # 从字符证据计算引用数（assemble_series_bible中的打分逻辑）
        evidence_count = len(character.get('evidence_event_ids', []))
        thread_count = len(thread_char_sets.get(char_id, set()))
        relationship_count = len([
            rid for rid, rel in relationship_by_id.items()
            if char_id in rel.get('character_ids', [])
        ])
        score_line = (
            f"- 重要性得分：{score:.3f} "
            f"（事件证据×{evidence_count}、"
            f"关系×{relationship_count}、"
            f"故事线×{thread_count}）"
        )
        lines.extend([
