...
消费 window_summaries (VLM 逐窗分析结果), 对每个窗口执行:
  ...
  6. 角色出场介绍卡片检查 (source="title_card") — trigger #6
  7. 低置信度时设置 enrichment_triggered=true 并建议补充动作
...
class ConfidenceCheckStage(Stage):
    ...
    def prepare(self, bus: ArtifactBus) -> list[Task]:
        ...
        if ref is None:
            raise RuntimeError("上游 window_summaries 产物未找到 (vlm_analysis/window_analysis 均无产物)")
    ...
    def execute(self, bus: ArtifactBus, tasks: list[Task]) -> list[Artifact]:
        ...
        for i, rec in enumerate(records):
            ...
            if db is not None:
                ...
                try:
                    ...
                    logger.warning(
                        "写入 vlm_confidence_log 失败 [window=%s]: %s",
        ...
        ref = bus.put(
            ...
            inputs=(
                [bus.latest("vlm_analysis") or bus.latest("window_analysis")]
                if (bus.latest("vlm_analysis") or bus.latest("window_analysis"))
                else []
            ),
        )
    ...
    def _assess_window(
        ...
        # ── 6. 角色参考注入检查 (trigger #6: 缺少出场介绍卡片) ──
        needs_character_ref = self._needs_character_reference(rec)
        ...
        enrichment_triggered = self._should_enrich(
            ...
            needs_character_reference=needs_character_ref,
        )
        ...
        if needs_character_ref:
            logger.info(
                "窗口 %s 角色缺少出场介绍卡片 (>2 角色), 建议注入角色参考表",
                window_id,
            )
    ...
    @staticmethod
    def _needs_character_reference(rec: dict[str, Any]) -> bool:
        """Check if character reference injection is needed (trigger #6).

        Returns True when:
          - Total characters > 2, AND
          - Any character from subjects lacks source="title_card"
          (i.e. at least one character appeared without an intro card).
        """
        characters = ConfidenceCheckStage._extract_characters(rec)
        if len(characters) <= 2:
            return False
        ...
        if not isinstance(subjects, list) or not subjects:
            # No subjects at all — all characters are from dialogue speakers,
            # which means no title_card source — need reference.
            return True

        for s in subjects:
            if isinstance(s, dict) and s.get("name"):
                if s.get("source") != "title_card":
                    return True
    ...
    def _suggest_actions(
        ...
        if needs_character_reference:
            actions.append(
                "inject_character_reference: 角色缺少出场介绍卡片 (>2 角色), "
                "建议注入角色参考表"
            )
    ...
    def _build_global_summary(
        ...
        return {
            ...
            "asr_recommended": any(
                any("trigger_asr" in action for action in a.get("suggested_actions", []))
