...
class Task:
    ...
    type: 任务类型 (如 vlm_analysis); payload: 任务参数字典
...
class Stage(ABC):
    ...
    def resolve_artifact_path(
        ...
        优先按 stage+name 精确匹配, 匹配不到时 fallback 到该 stage 最新产物;
        产物内容为含 "path" 键的字典时返回其中的路径, 否则返回产物自身路径。
        找不到产物时抛 ArtifactNotFoundError。
        """
        ref = bus.resolve(stage, name) or bus.latest(stage)
        if ref is None:
