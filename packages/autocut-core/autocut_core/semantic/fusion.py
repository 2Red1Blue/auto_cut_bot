"""vlm-analysis-fusion — 多源数据融合引擎。

DEPRECATED since v5.1 (VLM-first architecture): 多源融合函数
(merge_boundaries, cross_validate, enrich_subjects, confidence_weighted_merge)
不再被 vlm_analysis stage 调用。VLM 直接从视频提取所有信息，
不再需要 API/ASR/剧本的交叉验证。

保留:
  - DataFusion.merge_boundaries_with_scenes: 场景合并 (deterministic, 无 LLM)
  - DataFusion.inherit_subjects: 主体继承 (跨窗口, 无 LLM)
  - FusionManager: 窗口级融合状态管理
...
原架构 (v4.x):
  ...
  - ASR (asr_transcript): FunASR 转录文本 (仅文本, 不参与边界决策)

新架构 (v5.x VLM-first):
  - 第 1 层: 视觉场景检测 (PySceneDetect) — 帧级精确的镜头边界
  - 第 2 层: VLM 语义分析 — 滑动窗口内的内容理解, 注入 global_context
  - 第 3 层: confidence_check — 质量门控, 按需触发 ASR
"""

设计原则:
  - 镜头边界为锚点: VLM/API 事件映射到最近的镜头切点, 不做时间轴平权合并
  - ASR 降级为文本源: 时间戳不可靠, 不参与边界融合
...
class DataFusion:
    ...
    @staticmethod
    def merge_boundaries_with_scenes(
        scene_cuts: list[float] | None,
        vlm: list[dict[str, Any]] | None = None,
        api: list[dict[str, Any]] | None = None,
    ) -> list[dict[str, Any]]:
        """以镜头切点为锚点, 合并 VLM/API 事件到最近的镜头边界。

        scene_cuts: [0.0, 3.48, 5.24, 7.16, ...] 来自 PySceneDetect
        vlm: VLM 分析产出的 high/medium 置信度事件
        api: API 分镜/字幕边界

        合并策略:
          1. 镜头切点作为硬边界锚点 (source='scenedetect', confidence='high')
          2. VLM 事件映射到最近的镜头切点, 保留 original_time
          3. API 事件按时间重叠合并到镜头区间
          4. 不在任何镜头区间内的事件独立保留

        返回:
          [{"start": 0.0, "end": 3.48, "source": "scenedetect", "event_type": "scene_cut",
            "vlm_events": [...], "api_events": [...]}, ...]
        """
        ...
        if not cuts:
            # 无镜头切点 — 回退到传统平权合并
            return DataFusion.merge_boundaries(vlm=vlm, api=api, asr=None)

        # 1. 构建镜头区间
        for i, cut_start in enumerate(cuts):
            cut_end = cuts[i + 1] if i + 1 < len(cuts) else None
            entry: dict[str, Any] = {
                "start": cut_start,
                "end": cut_end,
                "source": "scenedetect",
        ...
        if vlm:
            for event in vlm:
                if not isinstance(event, dict):
                    continue
                event_start = float(event.get("start_time", event.get("start", 0)))
                event_end = float(event.get("end_time", event.get("end", event_start + 1)))
                if event_start >= event_end:
                    continue

                # 找到包含此事件的镜头区间
                scene_idx = DataFusion._find_scene_index(result, event_start, event_end)
                if scene_idx is not None:
                    result[scene_idx]["vlm_events"].append({
                        "event_type": event.get("event_type", "unknown"),
                        "confidence": event.get("confidence", "low"),
                        "original_start": event_start,
                        "original_end": event_end,
                        "description": event.get("description", ""),
                        "subjects": DataFusion._normalize_subjects_list(event.get("subjects", [])),
                    })
                    result[scene_idx]["subjects"] = DataFusion._merge_subject_lists(
                        result[scene_idx]["subjects"],
                        DataFusion._normalize_subjects_list(event.get("subjects", [])),
                    )
                    # 提升置信度
                    result[scene_idx]["confidence"] = DataFusion._max_confidence(
                        result[scene_idx]["confidence"],
        ...
        if api:
            for event in api:
                ...
                scene_idx = DataFusion._find_scene_index(result, event_start, event_end)
                if scene_idx is not None:
                    result[scene_idx]["api_events"].append({
                        "event_type": event.get("event_type", "unknown"),
                        "confidence": event.get("confidence", "low"),
                        "start": event_start,
                        "end": event_end,
        ...
        # 4. 清理空 interval
        result = [r for r in result if r["end"] is not None]
    ...
    def _find_scene_index(
        ...
        """找到包含事件 [event_start, event_end] 的镜头区间索引。

        使用重叠度作为匹配标准: 事件与镜头区间的重叠 > 50% 即匹配。
        """
        best_idx = None
        best_overlap = 0.0
        ...
        for i, scene in enumerate(scenes):
            ...
            # 计算重叠区间
            overlap_start = max(scene_start, event_start)
            overlap_end = min(scene_end, event_end)
            if overlap_start >= overlap_end:
                continue

            overlap = overlap_end - overlap_start
            event_dur = event_end - event_start
            overlap_ratio = overlap / event_dur if event_dur > 0 else 0
...
def merge_boundaries_with_scenes(
    ...
    """以镜头切点为锚点, 合并 VLM/API 事件到最近的镜头边界。"""
    return DataFusion.merge_boundaries_with_scenes(scene_cuts, vlm, api)


# ═══════════════════════════════════════════════════════════════════════════════
# 便捷函数 — 传统平权合并 (向后兼容, 不推荐新代码使用)
# ═══════════════════════════════════════════════════════════════════════════════
...
def apply_vlm_arbitration(
    ...
    """将 VLM 字幕仲裁结果写入 subtitles 表。

    VLM 输出的每条 dialogue_and_text 包含 source_accuracy:
      - agreement: subtitle_match | subtitle_divergence | no_subtitle | screen_text_only
      - chosen_source: subtitle | audio | screen_text
      - reason: 选择理由
      - vlm_override_text: 所有源都不准确时 VLM 自己的判断

    写入策略:
      1. 最终文本 (text) 来自 chosen_source 对应的版本
      2. 统计各来源准确率
      3. source 字段记录 chosen_source

    返回统计字典: {subtitle_match, subtitle_chosen, audio_chosen, ...}
    """
    ...
    stats: dict[str, Any] = {
        "total": 0,
        "subtitle_match": 0,
        "subtitle_divergence": 0,
        "no_subtitle": 0,
        "screen_text_only": 0,
        "subtitle_chosen": 0,
        "audio_chosen": 0,
        "screen_text_chosen": 0,
        "override": 0,
        "uncertain": 0,
    }
    ...
    for item in vlm_dialogues:
        ...
        # 统计 — 兼容新旧两种 agreement/chosen_source 类型
        stats["total"] += 1
        if agreement in ("subtitle_match", "both_match"):
            stats["subtitle_match"] += 1
        elif agreement in ("subtitle_divergence", "minor_divergence", "major_divergence", "all_diverge", "vlm_override"):
            stats["subtitle_divergence"] += 1
        elif agreement in ("no_subtitle", "asr_only", "script_only"):
            stats["no_subtitle"] += 1
        elif agreement in ("screen_text_only", "api_only"):
            stats["screen_text_only"] += 1

        if chosen in ("subtitle", "api", "both", "script"):
            stats["subtitle_chosen"] += 1
        elif chosen in ("audio", "asr"):
            stats["audio_chosen"] += 1
        elif chosen in ("screen_text",):
            stats["screen_text_chosen"] += 1
        elif chosen in ("vlm",):
            stats["override"] += 1
        else:
            stats["uncertain"] += 1
