...
def compute_option_character_ids(
    option: dict[str, Any],
    thread_by_id: dict[str, dict[str, Any]],
) -> list[str]:
    """Compute valid character IDs for a subarc option from its threads."""
    return sorted(
        {
            character_id
            for thread_id in option.get("story_thread_ids", [])
...
def compute_option_relationship_ids(
    option_character_ids: list[str],
    relationships: list[dict[str, Any]],
) -> list[str]:
    """Compute valid relationship IDs whose characters are all in the option."""
    char_set = set(option_character_ids)
    return sorted(
        item["id"]
        for item in relationships
        if set(item.get("character_ids", []) or []) <= char_set
        and len(item.get("character_ids", []) or []) >= 2
    )


# Fields whose values are fully determined by upstream code and injected
# after LLM generation.  The schema keeps them as loose types so that
# strict-mode validation passes when the LLM outputs placeholders.
_SYSTEM_INJECTED_STORY_FIELDS = frozenset({
    "story_id",
    "subarc_option_id",
    "story_thread_ids",
...
def build_broad_catalog_schema(
    ...
    *,
    ...
    System-injected fields (IDs, beats, seconds, etc.) use loose types
    so the LLM can output placeholders.  The canonicalize layer and
    _assemble_catalog overwrite these with deterministic values before
    ...
    for option in options:
        ...
        # ── System-injected fields: loose types (no const/enum) ──
        props["subarc_option_id"] = {"type": "string"}
        ...
        props["subarc_start_beat_id"] = {"type": "string"}
        props["subarc_end_beat_id"] = {"type": "string"}
        props["estimated_source_seconds"] = {"type": "number"}
        props["duration_feasibility"] = {"type": "string"}

        option_character_ids = compute_option_character_ids(option, thread_by_id)
        option_relationship_ids = compute_option_relationship_ids(
            option_character_ids, relationships,
        )
        # character_ids: keep nonempty constraint but drop enum
        props["character_ids"] = {
            "type": "array",
            "items": {"type": "string"},
            "minItems": 1,
        }
        props["relationship_ids"] = {
            "type": "array",
            "items": {"type": "string"},
        }
        props["required_fact_ids"] = {
            "type": "array",
            "items": {"type": "string"},
        }

        # ── LLM-selected fields: keep enum (small sets, LLM can handle) ──
...
def broad_catalog_prompt(context: dict[str, Any]) -> str:
    ...
    return (
        "\n\n【Broad Story · 创作指引】\n"
        "你的任务是为本子弧撰写叙事描述。以下字段由系统自动填充，"
        "请输出空字符串 \"\" 或空数组 [] 作为占位符，不要尝试填写：\n"
        "story_id, subarc_option_id, story_thread_ids, source_thread_beat_ids, "
        "subarc_start_beat_id, subarc_end_beat_id, required_bridge_beat_ids, "
        "evidence_event_ids, estimated_source_seconds, duration_feasibility, "
        "character_ids, relationship_ids, required_fact_ids。\n\n"
        "请将全部注意力集中在以下创作字段上：\n"
        "- title: 故事标题（简短有力）\n"
        "- logline: 一句话梗概\n"
        "- central_question: 核心悬念\n"
        "- start_state / end_state: 起始/终止状态描述\n"
        "- payoff_summary: 情感/叙事兑现\n"
        "- open_hook_summary: 留给后续的悬念\n"
        "- overlap_notes: 与其他子弧的重叠说明（可留空）\n"
        "- scores: 7 个维度的 1-10 评分\n"
        "- recommendation_reason: 推荐理由\n\n"
        "suggested_highlight_candidate_ids 只能从 schema 枚举中选；"
        "suggested_hook_candidate_ids 同理。无合适候选时输出空数组。\n"
        f"本子弧对应 Option：{recommended}。"
    )
