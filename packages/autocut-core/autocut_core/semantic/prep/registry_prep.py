        ...
        "episode_thread_index": [
            {
                ...
                # Phase 1: 恢复 opening/ending_state（之前被丢弃），辅助 LLM 理解角色弧光
                "opening_state": item.get("opening_state", ""),
                "ending_state": item.get("ending_state", ""),
                "event_count": len(item.get("event_ids", []) or []),
                "character_mentions": item.get("character_mentions", []),
            }
            for item in episodes
        ],
