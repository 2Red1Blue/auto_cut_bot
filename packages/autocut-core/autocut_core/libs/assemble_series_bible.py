...
def assemble_bible(
    *,
        ...
        final_threads.append(
            {
                ...
                "thread_kind": registry_thread.get("thread_kind", "arc"),
                "title": registry_thread["title"],
                "premise": registry_thread["premise"],
                "summary": registry_thread["premise"],
    ...
    # ── v1.4: entity_importance heuristic scoring ──
    # Score characters by objective signals:
    #   - evidence_event_ids count (more events = more screen presence)
    #   - participation in resolved threads
    #   - entity_type bonus (individual > group/creature)
    #   - first_event recency (earlier appearance = core character)
    thread_char_sets: dict[str, set[str]] = {}
    for ft in final_threads:
        for cid in ft.get("character_ids", []):
            thread_char_sets.setdefault(cid, set()).add(ft["id"])
    total_events_count = len(events)
    entity_importance: dict[str, dict] = {}
    for cid, ch in character_by_id.items():
        evidence_count = len(ch.get("evidence_event_ids", []))
        # Normalize evidence count to 0-1
        event_score = min(evidence_count / max(total_events_count * 0.3, 1), 1.0)
        # Thread participation
        thread_count = len(thread_char_sets.get(cid, set()))
        thread_score = min(thread_count / max(len(final_threads), 1), 1.0)
        # Entity type bonus
        type_bonus = 0.1 if ch.get("entity_type") == "individual" else 0.0
        # First event (earlier = more core)
        first_ev = ch.get("first_event_id", "")
        first_ep = 0
        if first_ev in event_by_id:
            first_ep = int(event_by_id[first_ev].get("episode", 999))
        earliness_score = max(0, 1.0 - (first_ep - 1) / max(len(source_episode_ids), 1))
        # Weighted composite
        score = round(
            0.5 * event_score + 0.3 * thread_score + 0.1 * type_bonus + 0.1 * earliness_score,
            3,
        )
        score = min(max(score, 0.0), 1.0)
        reasons = []
        if evidence_count:
            reasons.append(f"{evidence_count} evidence events")
        if thread_count:
            reasons.append(f"in {thread_count} thread(s)")
        if ch.get("entity_type"):
            reasons.append(ch["entity_type"])
        entity_importance[cid] = {
            "score": score,
            "reason": "; ".join(reasons) if reasons else "background character",
        }
    # main_characters: top 12 individuals sorted by score descending
    sorted_chars = sorted(
    ...
    main_characters = [
        cid for cid, info in sorted_chars
        if character_by_id.get(cid, {}).get("entity_type") == "individual"
    ][:12]
