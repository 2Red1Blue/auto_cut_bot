...
# Golden case is deployment-specific; no default bundled in open-source.
# Private deployments can set ``AUTOCUT_GOLDEN_CASE`` env var or pass an explicit path.
DEFAULT_GOLDEN_CASE_PATH: Path | None = None


def validate_golden_case(path: Path | None = None) -> list[str]:
    """Validate the project-level positive/negative editorial fixture.

    Returns an empty list when no golden case is configured (default in open-source).
    Private deployments can set AUTOCUT_GOLDEN_CASE env var or pass an explicit path.
    ...
    """
    # Explicit path > env var > default (None in open-source)
    if path is not None:
        target = path.expanduser().resolve()
    else:
        env_path = os.environ.get("AUTOCUT_GOLDEN_CASE")
        if env_path:
            target = Path(env_path).expanduser().resolve()
        elif DEFAULT_GOLDEN_CASE_PATH is not None:
            target = DEFAULT_GOLDEN_CASE_PATH.expanduser().resolve()
        else:
            return []  # no golden case configured
    errors: list[str] = []
    try:
