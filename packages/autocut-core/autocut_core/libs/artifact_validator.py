...
def _fuzzy_match_id(bad_id: str, known_ids: set[str], max_distance: int = 1) -> str | None:
    """Try to fuzzy match an invalid ID to a known ID with edit distance <= max_distance.
    Returns the matching ID if exactly one candidate is found, None otherwise.
    """
    import os
    if os.environ.get("ENABLE_ID_FUZZY_MATCH", "true").lower() == "false":
        return None
    # Only apply fuzzy matching to known ID prefixes to avoid false positives
    id_prefixes = ("event-", "char-", "rel-", "thread-", "q-", "fact-", "cand-", "src-", "win-")
    if not any(bad_id.startswith(prefix) for prefix in id_prefixes):
        return None
    candidates = []
    for valid_id in known_ids:
        # Fast pre-filter: length difference > max_distance can't match
        if abs(len(bad_id) - len(valid_id)) > max_distance:
            continue
        if _levenshtein_distance(bad_id, valid_id) <= max_distance:
...
def check_refs(
    ...
    """Check ID references, auto-fix single-character errors via fuzzy matching.
    Returns the number of auto-fixed IDs.
    """
    fixed_count = 0
    if fixes is None:
        fixes = []
    if not isinstance(values, list):
        ...
        return fixed_count
    str_values = [item for item in values if isinstance(item, str)]
    unknown = sorted(set(str_values) - known)
    if unknown:
        # Try fuzzy matching for each unknown ID
        for bad_id in list(unknown):
            match = _fuzzy_match_id(bad_id, known)
            if match is not None:
                # Auto-fix ALL occurrences of this bad_id in-place
                for i, v in enumerate(values):
                    if v == bad_id:
                        values[i] = match
                unknown.remove(bad_id)
                fixed_count += 1
                fixes.append({
                    "field": where,
                    "original_id": bad_id,
                    "fixed_id": match,
                    "edit_distance": _levenshtein_distance(bad_id, match),
                })
        if unknown:
...
def fixup_fuzzy_ids_in_value(value: Any, known_ids: set[str], fixes: list[dict[str, Any]] | None = None) -> int:
    """Recursively walk a value and auto-fix single-character ID typos in-place.
    ...
    if isinstance(value, list):
        for i, item in enumerate(value):
            if isinstance(item, str) and item not in known_ids:
                match = _fuzzy_match_id(item, known_ids)
                if match is not None:
                    fixes.append({
                        "path": f"[{i}]",
                        "original_id": item,
                ...
                fixed += fixup_fuzzy_ids_in_value(item, known_ids, fixes)
    elif isinstance(value, dict):
        for key, val in value.items():
            if isinstance(val, str) and (key.endswith("_ids") or key in {"event_id", "character_id", "thread_id", "relationship_id", "fact_id", "question_id"}):
                if val not in known_ids:
                    match = _fuzzy_match_id(val, known_ids)
                    if match is not None:
                        fixes.append({
                            "path": f".{key}",
                            "original_id": val,
                ...
                sub_fixes = []
                sub_fixed = fixup_fuzzy_ids_in_value(val, known_ids, sub_fixes)
                for f in sub_fixes:
                    f["path"] = f".{key}{f['path']}"
...
def validate(
    ...
    for index, window in enumerate(windows):
        schema_errors = validate_task_response("vlm_analysis", window) or validate_task_response("window_analysis", window)
