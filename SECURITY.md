...
If you discover a security vulnerability in auto_cut_bot, please report it by:
...
- ✅ Never run auto_cut_bot as root
...
- Config files and API keys (`~/.auto_cut_bot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run auto_cut_bot with a dedicated user account
...
**WhatsApp:**
- Keep the neonize session database under `~/.auto_cut_bot/whatsapp-auth` secure (mode 0700).
- Use `auto_cut_bot channels login whatsapp --force` to remove and recreate the local session database when rotating linked devices.
...
pip install --upgrade auto-cut-bot-ai
```
...
- Subscribe to security advisories for auto_cut_bot and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install auto-cut-bot-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash auto_cut_bot
   sudo -u auto_cut_bot auto_cut_bot gateway
   ```

3. **Set Proper Permissions**
   ```bash
   chmod 700 ~/.auto_cut_bot
   chmod 600 ~/.auto_cut_bot/config.json
   chmod 700 ~/.auto_cut_bot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.auto_cut_bot/logs/auto_cut_bot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.auto_cut_bot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.auto_cut_bot/logs/auto_cut_bot.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/auto_cut_bot/security/advisories
- Release Notes: https://github.com/HKUDS/auto_cut_bot/releases
