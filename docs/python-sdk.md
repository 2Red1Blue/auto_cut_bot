...
Use auto_cut_bot as a Python library. The SDK gives you the same agent runtime used
...
- auto_cut_bot SDK runs an agent around a model.
...
Install and configure auto_cut_bot first. If you have not done that yet, follow the
...
`Nanobot.from_config()` reuses your normal `~/.auto_cut_bot/config.json` and
`~/.auto_cut_bot/workspace/`. Provider, model, tools, memory, and session behavior
...
`auto_cut_bot status` should show the config path, workspace path, active model or
...
from auto_cut_bot import Nanobot
...
```python
from auto_cut_bot import STREAM_EVENT_TEXT_DELTA
...
Save this as `sdk_demo.py` after `auto_cut_bot agent -m "Hello!"` works:
...
async def main() -> None:
    prompt = " ".join(sys.argv[1:]) or "Explain what auto_cut_bot is in one paragraph."
    ...
    async with Nanobot.from_config() as bot:
        ...
        async for event in bot.stream(prompt, session_key=session_key):
            if event.type == STREAM_EVENT_TEXT_DELTA:
                ...
                raise RuntimeError(event.error or "auto_cut_bot run failed")
...
workspace: /Users/alice/.auto_cut_bot/workspace
...
| Memory | Long-term memory files managed by auto_cut_bot. |
...
auto_cut_bot has two programming surfaces:

| Use | Choose | Why |
|-----|--------|-----|
| Python code running in the same process as auto_cut_bot | Python SDK | Direct access to `RunResult`, sessions, memory, runtime helpers, hooks, and stream events. |
...
auto_cut_bot directly.

The OpenAI-compatible API is best when you already have an HTTP client, want
process isolation, or need to call auto_cut_bot from a non-Python service.
...
Use a custom config when you run multiple auto_cut_bot instances or test an isolated
setup:
...
The config controls what auto_cut_bot may use. The workspace is where auto_cut_bot keeps
...
provider and model work with `auto_cut_bot agent -m "Hello!"` first.
...
become auto_cut_bot session history. Ingesting a transcript does not call the model,
...
metrics, tracing, or output post-processing without modifying auto_cut_bot internals:

```python
from auto_cut_bot.agent import AgentHook, AgentHookContext
...
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.auto_cut_bot/config.json`. |
...
auto_cut_bot agent loop. A context provider receives a `RequestContext` before each
model turn and may return one or more `RuntimeContextBlock` values. Use
`attributes` for caller-owned routing data; auto_cut_bot keeps it separate from
...
```python
from auto_cut_bot.agent import AgentHook
