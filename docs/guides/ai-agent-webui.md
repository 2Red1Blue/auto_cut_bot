# How to Use an AI Agent WebUI with auto_cut_bot

auto_cut_bot includes a browser WebUI for persistent chat sessions, visible agent
...
When auto_cut_bot edits a file, the WebUI activity timeline can show the changed
...
- Use `auto_cut_bot gateway --background` when you do not want to keep a terminal open, then open the
  configured WebUI URL in a browser.
- Use `auto_cut_bot gateway status`, `logs`, `restart`, and `stop` to manage a
  background gateway.
