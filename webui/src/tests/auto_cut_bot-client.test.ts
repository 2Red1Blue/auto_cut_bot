...
describe("NanobotClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("auto_cut_bot-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("auto_cut_bot-host://engine/");
    expect(lastSocket().url).toBe("host:auto_cut_bot-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    expect(openSocket).toHaveBeenCalledWith("auto_cut_bot-host://engine/");
