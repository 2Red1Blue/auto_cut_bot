...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "auto_cut_bot-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('auto_cut_bot_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('auto_cut_bot_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("auto_cut_bot_debug_ws")?.trim().toLowerCase() ?? "";
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[auto_cut_bot ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[auto_cut_bot ws inbound]", summarizeInboundWsPayload(parsed));
    }
