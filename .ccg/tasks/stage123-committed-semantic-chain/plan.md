...
9. Switch Pipeline and Agent runtimes to the three shared commands.  In the same
   migration wave delete the old fixture semantic command, v2 adapter,
   production mega-facade and dead Stage 1-3 prototype consumers/tests.
...
- Delete after cutover: fixture semantic chain command/adapters, old v2 typed
  readers and production code paths that import the prototype.
...
cross-layer regression test is added.  Compatibility shims may re-export a new
type for one release, but may not translate old requests, mint authority or
invoke an old builder.
