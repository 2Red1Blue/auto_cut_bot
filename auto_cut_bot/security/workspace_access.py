...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses auto_cut_bot application-level guards.",
    )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("AUTO_CUT_BOT_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("AUTO_CUT_BOT_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("AUTO_CUT_BOT_SANDBOX_ENFORCED")
