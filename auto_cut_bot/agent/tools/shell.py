...
from auto_cut_bot.agent.tools.sandbox import wrap_command
...
from auto_cut_bot.security.workspace_access import current_scope_allows_loopback, current_tool_workspace
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to auto_cut_bot internal state files (#2989).
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments: list[str] = []
        if self.path_prepend:
            env["AUTO_CUT_BOT_PATH_PREPEND"] = self.path_prepend
            segments.append("$AUTO_CUT_BOT_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["AUTO_CUT_BOT_PATH_APPEND"] = self.path_append
            segments.append("$AUTO_CUT_BOT_PATH_APPEND")
    ...
    def _guard_command(
        ...
        *,
        ...
        from auto_cut_bot.security.network import contains_internal_url
