...
from auto_cut_bot.agent.hook import AgentHook, AgentHookContext, AgentRunHookContext
from auto_cut_bot.agent.tools.execution import execute_tool_calls
from auto_cut_bot.agent.tools.registry import ToolRegistry
from auto_cut_bot.llm_usage.context import (
    LLMUsageSource,
...
from auto_cut_bot.providers.conversation_state import ProviderConversationStateController
from auto_cut_bot.session.summary import SessionSummaryCheckpoint
...
class AgentRunner:
    ...
    async def _request_model(
        ...
        *,
        ...
        # (AUTO_CUT_BOT_STREAM_IDLE_TIMEOUT_S), but a stream that keeps producing
        # very slow deltas can still run forever. Use a more generous wall-clock
        # timeout for streaming while preserving AUTO_CUT_BOT_LLM_TIMEOUT_S=0 as an
    ...
    def _resolve_llm_timeout_s(spec: AgentRunSpec) -> float | None:
        ...
        if timeout_s is None:
            ...
            # Set AUTO_CUT_BOT_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("AUTO_CUT_BOT_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
