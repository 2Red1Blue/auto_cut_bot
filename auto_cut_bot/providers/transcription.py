...
``auto_cut_bot.audio.transcription``.
"""
...
class FunASRTranscriptionProvider:
    """Voice transcription provider using a self-hosted FunASR /recognition endpoint."""
    ...
    def __init__(
        ...
        base = api_base or os.environ.get("FUNASR_BASE_URL") or "http://localhost:10095"
        self.api_url = base.rstrip("/") + "/recognition"
        self.language = language or None
        self.model = model or "sensevoice"
        logger.debug("FunASR transcription endpoint: {}", self.api_url)
    ...
    async def transcribe(self, file_path: str | Path) -> str:
        ...
        try:
            ...
            logger.exception("FunASR transcription error: cannot read audio file: {}", e)
            return ""
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                async with httpx.AsyncClient() as client:
                    files: dict[str, Any] = {
                        "audio": (path.name, data, _audio_mime_type(path)),
                    }
                    if self.language:
                        files["language"] = (None, self.language)
                    response = await client.post(
                        self.api_url, files=files, timeout=600.0,
                    )
                response.raise_for_status()
                payload = response.json()
                text = payload.get("text", "")
                ...
                if attempt < _MAX_RETRIES:
                    logger.warning(
                        "FunASR transcription transient error (attempt {}/{}): {}",
                        attempt + 1, _MAX_RETRIES + 1, e,
                    )
                    await asyncio.sleep(_BACKOFF_S[attempt])
                    continue
                logger.exception("FunASR transcription error after {} attempts", _MAX_RETRIES + 1)
                return ""
            except httpx.HTTPStatusError as e:
                logger.error(
                    "FunASR transcription HTTP {}: {}",
                    e.response.status_code, e.response.text[:500],
                )
                return ""
            except Exception as e:
                logger.exception("FunASR transcription error: {}", e)
                return ""
        return ""
