...
from auto_cut_bot.config.timezone import detect_system_timezone
from auto_cut_bot.config_base import Base
from auto_cut_bot.cron.types import CronSchedule
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by auto_cut_bot.audio.transcription_registry.
    model: str | None = None
...
class AgentDefaults(Base):
    ...
    bot_name: str = "auto_cut_bot"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
...
class ProviderConfig(Base):
    ...
    # auto_cut_bot/providers/openai_compat_provider.py. Kept duplicated here to
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("auto_cut_bot.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("auto_cut_bot.agent.tools.shell", "ExecToolConfig"))
    file: FileToolsConfig = Field(default_factory=lambda: _lazy_default("auto_cut_bot.agent.tools.filesystem", "FileToolsConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("auto_cut_bot.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("auto_cut_bot.agent.tools.self", "MyToolConfig"))
...
class Config(BaseSettings):
    """Root configuration for auto_cut_bot."""
    ...
    def _match_provider(
        ...
        *,
        ...
        from auto_cut_bot.providers.registry import (
            PROVIDERS,
            find_by_name,
        )
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from auto_cut_bot.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
