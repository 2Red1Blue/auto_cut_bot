...
try:
    import auto_cut_bot.channels.dingtalk.runtime as dingtalk_module
...
from auto_cut_bot.channels.dingtalk.runtime import (
    DingTalkChannel,
    DingTalkConfig,
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/auto_cut_bot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/auto_cut_bot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "auto_cut_bot.config.paths.get_media_dir",
