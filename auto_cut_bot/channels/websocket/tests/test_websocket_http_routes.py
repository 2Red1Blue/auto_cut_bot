...
async def test_webui_skills_route_requires_token_and_hides_paths(
    ...
    (unavailable_skill / "SKILL.md").write_text(
        "\n".join([
            ...
            "      env:",
            "        - DEFINITELY_MISSING_AUTO_CUT_BOT_SKILL_ENV",
            "---",
    ...
    try:
        ...
        assert unavailable["unavailable_reason"] == (
            ...
            "ENV: DEFINITELY_MISSING_AUTO_CUT_BOT_SKILL_ENV"
        )
        ...
        assert detail_body["requirements"] == {
            ...
            "missing_env": ["DEFINITELY_MISSING_AUTO_CUT_BOT_SKILL_ENV"],
        }
...
async def test_auto_cut_bot_feature_routes_require_token_and_enable(
    ...
    try:
        ...
        enabled = await _http_get(
            "http://127.0.0.1:29916/api/settings/auto_cut_bot-features/enable?name=matrix",
            headers=auth,
        )
        ...
        disabled_websocket = await _http_get(
            "http://127.0.0.1:29916/api/settings/auto_cut_bot-features/disable?name=websocket",
            headers=auth,
        )
        ...
        disabled = await _http_get(
            "http://127.0.0.1:29916/api/settings/auto_cut_bot-features/disable?name=matrix",
            headers=auth,
        )
...
async def test_auto_cut_bot_feature_remote_install_requires_opt_in(
    ...
    channel = _ch(bus, session_manager=_seed_session(tmp_path), port=_free_port())
    token = channel.gateway.tokens.issue_api_token(300)
    path = "/api/settings/auto_cut_bot-features/enable?name=matrix"
...
async def test_auto_cut_bot_feature_remote_disable_does_not_need_install_policy(
    ...
    request = _FakeReq(
        {"Authorization": f"Bearer {token}"},
        path="/api/settings/auto_cut_bot-features/disable?name=matrix",
    )
...
async def test_mcp_presets_routes_require_token_and_return_payload(
    ...
    monkeypatch.setattr(
        "auto_cut_bot.webui.settings_routes.request_mcp_reload",
        _hot_reload,
    )
