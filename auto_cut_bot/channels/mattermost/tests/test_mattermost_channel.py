...
from auto_cut_bot.channels.mattermost.manifest import SETUP_SPEC
from auto_cut_bot.channels.mattermost.runtime import (
    MATTERMOST_MAX_MESSAGE_LEN,
    MattermostChannel,
    MattermostConfig,
)
from auto_cut_bot.pairing import PAIRING_CODE_META_KEY
...
async def test_start_identifies_bot():
    ...
    assert channel._self_username == "auto_cut_bot"
...
async def test_posted_event_routes_to_handle_message():
    ...
    channel._self_username = "auto_cut_bot"
            ...
            with patch.object(channel, "_should_respond_in_channel", return_value=True):
                ws_msg = {
                    ...
                    "data": {
                        ...
                        "post": json.dumps({
                            "id": "p1", "user_id": "u1",
                            "channel_id": "c1", "message": "@auto_cut_bot hello there", "root_id": "",
                        }),
                    },
                    "broadcast": {},
                }
...
async def test_group_policy_mention():
    ...
    assert channel._should_respond_in_channel("@auto_cut_bot hello", "c1") is True
...
async def test_group_policy_in_thread_defaults_to_group_policy():
    ...
    assert channel._should_respond_in_channel("@auto_cut_bot hello", "c1", in_thread=False) is True
    ...
    assert channel._should_respond_in_channel("@auto_cut_bot hello", "c1", in_thread=True) is True
...
async def test_send_with_file_upload():
    ...
    with patch("auto_cut_bot.channels.mattermost.runtime.Path.exists", return_value=True):
        with patch("auto_cut_bot.channels.mattermost.runtime.Path.read_bytes", return_value=b"data"):
            msg = OutboundMessage(
                channel="mattermost",
                chat_id="chan_1",
                content="with file",
        ...
        with patch.object(channel, "_is_allowed", AsyncMock(return_value=True)):
            ws_msg = {
                ...
                "data": {
                    ...
                    "post": json.dumps({
                        "id": "post_1", "user_id": "u1",
                        "channel_id": "c1", "message": "@auto_cut_bot start thread",
                        "root_id": "",
                    }),
                },
                "broadcast": {},
...
async def test_dm_allowlist_accepts_pairing_approval():
    ...
    with patch("auto_cut_bot.channels.mattermost.runtime.is_approved", return_value=True):
...
def test_is_mentioned_exact():
    channel, fake = _make_channel({"groupPolicy": "mention"})
    channel._self_username = "auto_cut_bot"
    assert channel._is_mentioned("hello @auto_cut_bot how are you") is True
    assert channel._is_mentioned("hello @auto_cut_botty") is False
    assert channel._is_mentioned("@auto_cut_bot_extra") is False
...
def test_is_mentioned_no_username():
    ...
    assert channel._is_mentioned("hello @auto_cut_bot") is False
...
def test_message_splitting():
    from auto_cut_bot.utils.helpers import split_message
    short = "short message"
