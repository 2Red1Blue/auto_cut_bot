...
def has_channel_package(name: str) -> bool:
    ...
    if _CHANNEL_PACKAGE_NAME.fullmatch(name) is None:
        return False
    return files("auto_cut_bot.channels").joinpath(name, "manifest.py").is_file()
...
def load_channel_package(name: str) -> ChannelPlugin | None:
    ...
    module_name = f"auto_cut_bot.channels.{name}.manifest"
    module = importlib.import_module(module_name)
    ...
    package_name = f"auto_cut_bot.channels.{name}"
    package_root = files("auto_cut_bot.channels").joinpath(name)
    ...
    if plugin.webui is not None:
        webui_entry = files("auto_cut_bot.channels").joinpath(name, *plugin.webui.split("/"))
