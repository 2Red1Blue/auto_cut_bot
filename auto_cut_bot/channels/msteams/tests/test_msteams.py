...
try:
    import auto_cut_bot.channels.msteams.runtime as msteams_module
...
if not MSTEAMS_AVAILABLE:
    pytest.skip(
        ...
        "Run: auto_cut_bot plugins enable msteams",
...
from auto_cut_bot.channels.msteams.runtime import ConversationRef, MSTeamsChannel
...
def make_channel(tmp_path, monkeypatch):
    monkeypatch.setattr("auto_cut_bot.channels.msteams.runtime.get_workspace_path", lambda: tmp_path)
...
async def test_start_logs_install_hint_when_pyjwt_missing(make_channel, monkeypatch):
    ...
    assert errors == ["PyJWT not installed. Run: auto_cut_bot plugins enable msteams"]
