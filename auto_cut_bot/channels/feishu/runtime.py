...
from auto_cut_bot.channels.contracts import ChannelInstanceSpec
from auto_cut_bot.channels.feishu.config import FeishuConfig, feishu_default_config
from auto_cut_bot.channels.feishu.instances import (
    DEFAULT_INSTANCE_ID,
...
from auto_cut_bot.channels.feishu.websocket import get_feishu_ws_runner
from auto_cut_bot.command.router import normalize_command_text
from auto_cut_bot.config.paths import get_media_dir
from auto_cut_bot.pairing import clear_channel
...
def save_registration_result(
    ...
    *,
    ...
    default_name = (
        ...
        else f"auto_cut_bot {effective_instance_id}"
    )
...
class FeishuChannel(BaseChannel):
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        _LOGIN_CONSOLE.print("Authorize with the mobile app. auto_cut_bot will save the new bot credentials.\n")

        result = qr_register(initial_domain=self.config.domain or "feishu")
        if not result:
            _LOGIN_CONSOLE.print(
                "[yellow]Login was not completed.[/yellow] "
                "Run 'auto_cut_bot channels login feishu --force' to retry."
            )
            return False
    ...
    async def start(self) -> None:
        ...
        if not FEISHU_AVAILABLE:
            self.logger.error("SDK not installed. Run: auto_cut_bot plugins enable feishu")
            return

        if not self.config.app_id or not self.config.app_secret:
            self.logger.error(
                "app_id and app_secret not configured. "
                "Run 'auto_cut_bot channels login feishu' to set up via QR code."
            )
            return
