...
def is_teaser_eligible_highlight(candidate: dict[str, Any]) -> bool:
    """Return True if a highlight candidate qualifies as a teaser-eligible opening.

    Checks that the candidate is a highlight type with sufficient strength,
    positioned early enough in the source material.
    """
    ...
    return float(start) <= 300.0  # first 5 minutes