...
class Stage3BlueprintPipelineStage:
    def __init__(
        ...
        if semantic_authority is not None and type(semantic_authority) is not SemanticRunAuthority:  # noqa: E721
            raise PipelineRunValidationError("Stage 3 requires an exact semantic authority")
        if installed_profile is not None and semantic_authority is not None:
            raise PipelineRunValidationError("Stage 3 authority sources are mutually exclusive")
        self._store = store
    ...
    def _request(self, context: PipelineStageContext) -> BuildEditorialBlueprintRequest | None:
        ...
        if semantic is not None and (
            ...
            or stage3_policy != semantic.stage3_command_policy
        ):
            raise PipelineRunValidationError(
                "persisted Stage 1/2/3 policies differ from installed semantic authority"
            )
