...
_EXECUTION_PROFILE_SCHEMA_VERSION_V11 = "pipeline-execution-profile-v11"
_EXECUTION_PROFILE_SCHEMA_VERSION_V12 = "pipeline-execution-profile-v12"
...
class EvidenceReadLimits:
    ...
    def __post_init__(self) -> None:
        for name, value in (
            ...
            if type(value) is not int or not 1 <= value <= 9_007_199_254_740_991:  # noqa: E721
                raise PipelineRunValidationError(
                    f"evidence_read_limits.{name} must be a positive safe integer"
                )
        if self.max_blob_bytes > self.max_total_blob_bytes:
            raise PipelineRunValidationError(
                "evidence per-blob budget exceeds the total batch budget"
            )
    ...
    def canonical_hash(self) -> str:
        return (
            "sha256:"
            + hashlib.sha256(_canonical_json(self.to_mapping()).encode("utf-8")).hexdigest()
        )
...
class PipelineExecutionProfile:
    ...
    def __post_init__(self, _historical_read_token: object | None) -> None:
        ...
        if (
            ...
            not in {
                ...
                _EXECUTION_PROFILE_SCHEMA_VERSION_V11,
                _EXECUTION_PROFILE_SCHEMA_VERSION_V12,
            }
        ...
        if (
            ...
            raise PipelineRunValidationError(
                ...
                "or semantic-story execution profile v11"
            )
        if self.kernel_parser_strategy_version == "strict-semantic-pack-v4":
            ...
            raise PipelineRunValidationError(
                "legacy parser profile cannot claim an implementation field"
            )
        ...
        explicit_thinking = (
            self.adapter_strategy_version == DOUBAO_ARK_EXPLICIT_THINKING_ADAPTER_STRATEGY_VERSION
        )
        if explicit_thinking and self.schema_version not in {
            _EXECUTION_PROFILE_SCHEMA_VERSION_V10,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V11,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V12,
        }:
            raise PipelineRunValidationError(
                "explicit thinking requires execution profile v10, v11 or v12"
            )
        parameter_fields = (
            _REQUEST_PARAMETER_FIELDS | {"thinking_type"}
            if explicit_thinking
            else _REQUEST_PARAMETER_FIELDS
        )
        ...
        if explicit_thinking:
            ...
            if type(thinking_type) is not str or thinking_type not in {
                "enabled",
                "disabled",
                "auto",
            }:  # noqa: E721
        ...
        if self.schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V1:
            ...
            if (
                ...
                "doubao-ark-responses-stream",
                "doubao-seed-2-1-pro-260628",
                "doubao-ark-files-responses-stream-v5",
                "vlm-semantic-pack-v23-context-assisted-candidate-core",
                "strict-semantic-pack-v4",
                "doubao-generate-vlm-semantic-pack-v4-probe-then-parallel-3-v8",
            ):
                raise PipelineRunValidationError(
                    "semantic-story execution profile requires the registered V23 VLM authority"
                )
            if self.schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V11 and any(
                value is not None
                for value in (
                    self.media_preflight_policy_json,
                    self.media_preflight_policy_hash,
                ...
                raise PipelineRunValidationError(
                    "semantic-story execution profile cannot claim physical evidence policies"
                )
            if self.schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V12:
                self._decode_media_preflight_policy()
                self._decode_materialization_limits()
        ...
        if self.schema_version in {
            ...
            raise PipelineRunValidationError(
                "historical execution profiles cannot claim Stage 1 policy"
            )
        if self.schema_version in {
            _EXECUTION_PROFILE_SCHEMA_VERSION_V7,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V8,
            ...
            raise PipelineRunValidationError(
                "historical execution profiles cannot claim Stage 2 policy"
            )
        if self.schema_version in {
            _EXECUTION_PROFILE_SCHEMA_VERSION_V8,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V9,
            ...
            raise PipelineRunValidationError(
                "historical execution profiles cannot claim Stage 3 policy"
            )
        if self.schema_version in {
            _EXECUTION_PROFILE_SCHEMA_VERSION_V9,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V10,
            ...
            raise PipelineRunValidationError(
                "historical execution profiles cannot claim evidence read limits"
            )
    ...
    @classmethod
    def from_semantic_story_policies(
        cls,
        policy: DoubaoVlmRequestPolicy,
        *,
        retry_policy: GenerationRetryPolicy,
        ...
        """Freeze a V23 semantic run that continues through Story Stage 3.

        V11 deliberately contains no timed-media, render or publication fields.
        It combines the installed semantic VLM authority with the independently
        installed Stage 1-3 command policies without pretending the historical
        V3 narrative profile produced the V23 evidence.
        """
        from autocut_kernel.semantic_chain.editorial_command_policy import Stage3CommandPolicy
        from autocut_kernel.semantic_chain.stage1_command_policy import Stage1CommandPolicy
        from autocut_kernel.semantic_chain.story_design_command_policy import Stage2CommandPolicy
        ...
        if type(policy) is not DoubaoVlmRequestPolicy:  # noqa: E721
            raise PipelineRunValidationError("semantic-story profile requires exact Doubao policy")
        if type(retry_policy) is not GenerationRetryPolicy:  # noqa: E721
            raise PipelineRunValidationError("semantic-story profile requires exact retry policy")
        if type(stage1_policy) is not Stage1CommandPolicy:  # noqa: E721
            raise PipelineRunValidationError("semantic-story profile requires exact Stage 1 policy")
        if type(stage2_policy) is not Stage2CommandPolicy:  # noqa: E721
            raise PipelineRunValidationError("semantic-story profile requires exact Stage 2 policy")
        if type(stage3_policy) is not Stage3CommandPolicy:  # noqa: E721
            raise PipelineRunValidationError("semantic-story profile requires exact Stage 3 policy")
        return cls(
            provider_id=policy.provider_id,
            model_id=policy.model_id,
            adapter_strategy_version=policy.adapter_strategy_version,
            ...
            stage2_command_policy_json=_canonical_json(stage2_policy.to_mapping()),
            stage3_command_policy_json=_canonical_json(stage3_policy.to_mapping()),
            schema_version=_EXECUTION_PROFILE_SCHEMA_VERSION_V11,
        )

    @classmethod
    def from_semantic_story_media_policies(
        cls,
        policy: DoubaoVlmRequestPolicy,
        media_preflight_policy: LocalMediaPreflightPolicy,
        *,
        ...
        """Freeze the V23 semantic-story and physical-evidence policies together.

        Stage 4 remains separately authorized by its command and deliberately
        does not participate in this execution-profile contract or hash.
        """
        from autocut_kernel.semantic_chain.editorial_command_policy import Stage3CommandPolicy
        from autocut_kernel.semantic_chain.stage1_command_policy import Stage1CommandPolicy
        from autocut_kernel.semantic_chain.story_design_command_policy import Stage2CommandPolicy
        ...
        if type(policy) is not DoubaoVlmRequestPolicy:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact Doubao policy"
            )
        if type(media_preflight_policy) is not LocalMediaPreflightPolicy:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact media-preflight policy"
            )
        if type(retry_policy) is not GenerationRetryPolicy:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact retry policy"
            )
        if type(materialization_limits) is not MaterializationLimits:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact materialization limits"
            )
        if type(stage1_policy) is not Stage1CommandPolicy:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact Stage 1 policy"
            )
        if type(stage2_policy) is not Stage2CommandPolicy:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact Stage 2 policy"
            )
        if type(stage3_policy) is not Stage3CommandPolicy:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact Stage 3 policy"
            )
        if type(evidence_read_limits) is not EvidenceReadLimits:  # noqa: E721
            raise PipelineRunValidationError(
                "semantic-story-media profile requires exact EvidenceReadLimits"
            )
        return cls(
            provider_id=policy.provider_id,
            model_id=policy.model_id,
            ...
            stage3_command_policy_json=_canonical_json(stage3_policy.to_mapping()),
            evidence_read_limits_json=_canonical_json(evidence_read_limits.to_mapping()),
            schema_version=_EXECUTION_PROFILE_SCHEMA_VERSION_V12,
        )
    ...
    def from_mapping(cls, value: Mapping[str, object]) -> PipelineExecutionProfile:
        ...
        if schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V1:
            ...
            allowed = base_allowed | {"generation_retry_policy"}
        elif schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V11:
            allowed = base_allowed | {
        ...
        elif schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V12:
            allowed = base_allowed | {
        ...
        return cls(
            ...
            parser_contract_sha256=(
                _profile_text(
                    value["parser_contract_sha256"], "execution_profile.parser_contract_sha256"
                )
                if "parser_contract_sha256" in allowed
                else None
            ),
    ...
    @property
    def is_semantic_story(self) -> bool:
        """Whether V23 semantic evidence continues through Story Stage 3."""
    ...
    @property
    def is_semantic_story_media(self) -> bool:
        """Whether V23 semantic-story policies are joined to physical evidence."""

        return self.schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V12

    @property
    def has_executable_plan(self) -> bool:
        return self.has_media_preflight_policy or self.is_semantic_only or self.is_semantic_story
    ...
    def build_stage1_command_policy(self) -> Stage1CommandPolicy:
        ...
        if (
            ...
            raise PipelineRunValidationError(
                "Stage 1 requires persisted execution profile v6, v7, v8 or v9, "
                "or semantic-story profile v11"
            )
    ...
    def build_stage2_command_policy(self) -> Stage2CommandPolicy:
        ...
        if (
            ...
            raise PipelineRunValidationError(
                "Stage 2 requires persisted execution profile v7, v8 or v9, "
                "or semantic-story profile v11"
            )
        value = _decode_canonical_json(
    ...
    def build_stage3_command_policy(self) -> Stage3CommandPolicy:
        ...
        if (
            ...
            raise PipelineRunValidationError(
                "Stage 3 requires persisted execution profile v8 or v9, "
                "or semantic-story profile v11"
            )
        value = _decode_canonical_json(
    ...
    def _decode_materialization_limits(self) -> MaterializationLimits:
        ...
        try:
            return MaterializationLimits(
                ...
                timed_speech_max_request_bytes=cast(int, value["timed_speech_max_request_bytes"]),
    ...
    def to_evidence_read_limits(self) -> EvidenceReadLimits:
        ...
        if self.schema_version not in {
            ...
            raise PipelineRunValidationError("evidence reads require execution profile v9 or v12")
...
def _build_registered_doubao_policy(
    ...
    if profile.schema_version in {
        ...
        registered_mapping["stage1_command_policy"] = (
            profile.build_stage1_command_policy().to_mapping()
        )
        registered_mapping["stage2_command_policy"] = (
            profile.build_stage2_command_policy().to_mapping()
        )
        registered_mapping["stage3_command_policy"] = (
            profile.build_stage3_command_policy().to_mapping()
        )
        registered_mapping["evidence_read_limits"] = profile.to_evidence_read_limits().to_mapping()
    elif profile.schema_version == _EXECUTION_PROFILE_SCHEMA_VERSION_V11:
        registered_mapping["stage1_command_policy"] = (
            profile.build_stage1_command_policy().to_mapping()
        )
        registered_mapping["stage2_command_policy"] = (
...
class VlmFullStageRecomputeRequest:
    ...
    completion_scope: Literal["full_stage", "selected_only"] = "full_stage"
    ...
    def __post_init__(self) -> None:
        ...
        if self.completion_scope not in ("full_stage", "selected_only"):
            raise PipelineRunValidationError(
                "recompute completion_scope must be 'full_stage' or 'selected_only'"
            )
        ...
        if self.completion_scope == "selected_only" and len(self.episode_numbers) != 1:
            raise PipelineRunValidationError("selected_only v1 requires exactly one episode number")
        if self.completion_scope == "full_stage" and self.episode_numbers:
            raise PipelineRunValidationError("full_stage v1 does not accept episode_numbers")

    @classmethod
    def from_mapping(cls, value: Mapping[str, object]) -> VlmFullStageRecomputeRequest:
        common = {"base_run_id", "expected_version", "stage", "completion_scope"}
        scope = value.get("completion_scope")
        expected: set[str] = common | ({"episode_numbers"} if scope == "selected_only" else set())
        if set(value) != expected:
            raise PipelineRunValidationError(
                "recompute body fields do not match the selected completion_scope"
            )
        raw_numbers = value.get("episode_numbers", [])
        ...
        return cls(
            ...
            cast(Literal["full_stage", "selected_only"], value["completion_scope"]),
            tuple(cast(list[int], raw_numbers)),
        )
    ...
    def to_mapping(self) -> dict[str, object]:
        ...
        if self.completion_scope == "selected_only":
            result["episode_numbers"] = list(self.episode_numbers)
        return result

    @property
    def selected_episode_index(self) -> int | None:
        if self.completion_scope != "selected_only":
            return None
...
class MediaPreflightRecomputeRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.episode_numbers) != 1:
            raise PipelineRunValidationError("selected_only v1 requires exactly one episode number")
        if (
            type(self.retry_budget) is not int
            or not 0 <= self.retry_budget <= MEDIA_PREFLIGHT_RECOMPUTE_MAX_RETRY_BUDGET
        ):  # noqa: E721
            raise PipelineRunValidationError(
...
class PipelineRunSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.recompute_request is not None and type(self.recompute_request) not in (  # noqa: E721
    ...
    def to_mapping(self) -> dict[str, object]:
        return {
            ...
            "recompute_request": (
                None if self.recompute_request is None else self.recompute_request.to_mapping()
            ),
            "status": self.status,
...
class PipelineStageContext:
    ...
    def __post_init__(self) -> None:
        ...
        if self.command.stage == "vlm" and self.execution_profile.schema_version not in {
            _EXECUTION_PROFILE_SCHEMA_VERSION_V9,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V10,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V11,
            _EXECUTION_PROFILE_SCHEMA_VERSION_V12,
        ...
        if self.command.stage in (
            ...
            raise PipelineRunValidationError(
                "physical/story stages require execution profile v9 or v11 for story stages"
            )
        if (
            self.command.stage == "media_preflight"
            and self.execution_profile.schema_version
            not in {
                _EXECUTION_PROFILE_SCHEMA_VERSION_V9,
                _EXECUTION_PROFILE_SCHEMA_VERSION_V12,
            }
        ):
            raise PipelineRunValidationError("media-preflight requires execution profile v9 or v12")
