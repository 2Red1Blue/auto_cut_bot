...
from auto_cut_bot.agent.tools.mcp import request_mcp_reload
from auto_cut_bot.api.runtime import ApiRuntime, ApiStartOptions, api_runtime_paths
from auto_cut_bot.bus.queue import MessageBus
from auto_cut_bot.channels._setup import channel_setup_spec
from auto_cut_bot.channels.connect import ChannelConnectError
...
from auto_cut_bot.webui.http_utils import case_insensitive_header
from auto_cut_bot.webui.http_utils import is_local_browser_request as _is_local_browser_request
from auto_cut_bot.webui.http_utils import query_first as _query_first
from auto_cut_bot.webui.mcp_presets_api import mcp_presets_settings_action
...
class WebUISettingsRouter:
    ...
    async def dispatch(self, connection: Any, request: WsRequest, path: str) -> Response | None:
        ...
        if path == "/api/settings/auto_cut_bot-features":
            return await self._handle_settings_auto_cut_bot_features(request)
        if path == "/api/settings/auto_cut_bot-features/enable":
            return await self._handle_settings_auto_cut_bot_features_action(connection, request, "enable")
        if path == "/api/settings/auto_cut_bot-features/disable":
            return await self._handle_settings_auto_cut_bot_features_action(connection, request, "disable")
    ...
    async def _handle_settings_auto_cut_bot_features(self, request: WsRequest) -> Response:
        if not self._authorized(request):
            return self._unauthorized()
        try:
            payload = await asyncio.to_thread(auto_cut_bot_features_payload)
        except Exception:
            self.logger.exception("failed to load auto_cut_bot features")
            return self._error_response(500, "failed to load auto_cut_bot features")
        return self._json_response(self._with_channel_runtime_status(payload))

    async def _handle_settings_auto_cut_bot_features_action(
        self,
        connection: Any,
        request: WsRequest,
        action: str,
        ...
        try:
            ...
            message = getattr(e, "message", str(e))
            if status >= 500:
                self.logger.exception("auto_cut_bot feature action '{}' failed", action)
            return self._error_response(status, message)
        payload = await self._apply_auto_cut_bot_feature_runtime_change(
            action,
            self._query(request),
            payload,
        )
    ...
    async def _apply_auto_cut_bot_feature_runtime_change(
        self,
        action: str,
        query: QueryParams,
        payload: dict[str, Any],
    ...
    async def _handle_settings_channel_configure(
        ...
        if not enable:
            features = await asyncio.to_thread(auto_cut_bot_features_payload)
            features = self._with_channel_runtime_status(features)
            payload["auto_cut_bot_features"] = self._with_restart_state(features, section="runtime")
            return self._json_response(payload)
        ...
        features = await self._apply_auto_cut_bot_feature_runtime_change(
            "enable",
            feature_query,
            features,
        )
