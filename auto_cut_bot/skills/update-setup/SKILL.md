---
name: update-setup
description: "One-time setup wizard for the auto_cut_bot upgrade skill. Triggers: setup update, configure update, 设置更新, 初始化更新."
---
...
Use `exec` to run `auto_cut_bot --version`. Tell the user the current version.
...
answer, stop and ask the user to rerun this setup when they know how auto_cut_bot was
installed.
...
`question: "Where is your auto_cut_bot source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
4. Use `exec` to verify: `auto_cut_bot --version`
5. Tell the user the new version. Say: "Run `/restart` to restart auto_cut_bot and apply the update. If `/restart` is unavailable in this channel, restart the auto_cut_bot process manually."
```

## Step 5: Confirm

Only after `write_file` succeeds, tell the user:
"Upgrade skill created. Say 'upgrade auto_cut_bot' when you want to update."
