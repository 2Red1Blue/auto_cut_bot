...
from auto_cut_bot.gateway import GatewayStartOptions, build_gateway_command
...
class GatewayServiceInstaller:
    ...
    def _install_launchd(
        ...
        *,
        ...
        stdout_path = self.home / ".auto_cut_bot" / "logs" / f"{log_stem}.launchd.log"
        stderr_path = self.home / ".auto_cut_bot" / "logs" / f"{log_stem}.launchd.err.log"
        payload = {
            "Label": label,
...
def _launchd_label(name: str) -> str:
    if name.startswith("ai.auto_cut_bot."):
        return name
    suffix = _safe_service_name(name).removeprefix("auto_cut_bot-").replace("-", ".")
    return f"ai.auto_cut_bot.{suffix}"
...
def _safe_service_name(name: str) -> str:
    ...
    value = value.strip(".-")
    return value or "auto_cut_bot-gateway"
