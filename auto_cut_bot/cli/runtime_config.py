...
def _print_runtime_config_validation_error(
    ...
    *,
    ...
    from auto_cut_bot.config.errors import ConfigIssue, ConfigLoadError, validation_issues
...
def _status_command(config_path: Path) -> str:
    return f'auto_cut_bot status --config "{config_path}"'
...
def _print_model_setup_steps(config_path: Path) -> None:
    ...
    console.print(
        f"  WebUI: run [cyan]auto_cut_bot webui {escape(config_arg)}[/cyan], "
        "then open Settings → Models"
    )
    console.print(f"  CLI:   run [cyan]auto_cut_bot onboard --wizard {escape(config_arg)}[/cyan]")
...
def _load_inspection_config(
    ...
    from auto_cut_bot.config.errors import ConfigLoadError
    from auto_cut_bot.config.loader import get_config_path, load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from auto_cut_bot.config.paths import get_cron_dir
...
def _provider_setup_error(config: Config) -> str | None:
    ...
    from auto_cut_bot.providers.factory import validate_provider_setup
