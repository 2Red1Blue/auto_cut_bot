...
from auto_cut_bot.cli.log_control import _set_nanobot_logs
...
_CLASSIC_DEPENDENCIES = {
    "AgentLoop": ("auto_cut_bot.agent.loop", "AgentLoop"),
    "StreamRenderer": ("auto_cut_bot.cli.stream", "StreamRenderer"),
    "consume_restart_notice_from_env": (
        "auto_cut_bot.utils.restart",
        "consume_restart_notice_from_env",
    ),
    "is_default_workspace": ("auto_cut_bot.config.paths", "is_default_workspace"),
    "sync_workspace_templates": ("auto_cut_bot.utils.helpers", "sync_workspace_templates"),
}
...
def agent(
    ...
    if native_tui:
        from auto_cut_bot.cli.tui_launcher import TuiSessionError, TuiUnavailableError, launch_tui
        ...
        try:
            ...
            console.print("[dim]Use `auto_cut_bot agent --classic` only if you want the old prompt.[/dim]")
    ...
    from auto_cut_bot.cli.stream import ThinkingSpinner
    ...
    from auto_cut_bot.utils.helpers import sanitize_surrogates as _sanitize_surrogates
