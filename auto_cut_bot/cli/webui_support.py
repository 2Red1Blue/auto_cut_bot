...
from auto_cut_bot.webui.build import (
    BuildMode,
    WebUIBuildError,
    ensure_webui_bundle,
)
...
if TYPE_CHECKING:
    from auto_cut_bot.gateway.runtime import GatewayRuntime
...
def _confirm_webui_action(message: str, *, yes: bool) -> None:
    ...
    if not _cli_can_prompt():
        console.print(
            ...
            "`auto_cut_bot onboard --wizard`.[/red]"
        )
        raise typer.Exit(1)
...
def _resolve_webui_config_path(config: str | None) -> Path:
    """Resolve the config path used by ``auto_cut_bot webui`` and bind loader state."""
...
def _validate_gateway_startup(config: Config) -> str | None:
    ...
    try:
        ...
        retry_command = f'auto_cut_bot gateway --config "{config_path}"'
        _print_runtime_config_validation_error(
            exc,
...
def _gateway_health_ready(host: str, port: int, *, timeout_s: float = 0.4) -> bool:
    """Return whether the auto_cut_bot gateway health endpoint responds OK."""
    import json
    import urllib.error
...
def _print_foreground_port_conflict(
    *,
    ...
    console.print(
        "[red]Error: auto_cut_bot cannot start because one of its local ports is already in use.[/red]"
    )
    ...
    console.print()
    console.print("If this is an existing auto_cut_bot instance, use it or stop it first:")
    console.print("  [cyan]auto_cut_bot gateway status[/cyan]")
    console.print("  [cyan]auto_cut_bot gateway stop[/cyan]")
    console.print(
...
def _print_webui_foreground_lifecycle(*, attached: bool) -> None:
    ...
    console.print()
    if attached:
        console.print("[green]auto_cut_bot is attached to the existing gateway.[/green]")
    else:
        console.print("[green]auto_cut_bot is running in this terminal.[/green]")
    console.print("[dim]Closing the browser does not stop channels or automations.[/dim]")
    console.print("[dim]Press Ctrl+C here to stop auto_cut_bot.[/dim]")
...
def _attach_to_background_gateway(
    ...
    *,
    ...
    try:
        ...
        console.print("\n[yellow]Stopping auto_cut_bot...[/yellow]")
        result = runtime.stop()
...
def _gateway_instance_command(
    ...
    *,
    ...
    parts = ["auto_cut_bot", "gateway", subcommand, "--config", str(config_path)]
    if workspace:
...
def _run_quick_start_for_webui(
    ...
    *,
    ...
    from auto_cut_bot.cli.onboard import run_quick_start_onboard
    ...
    try:
        ...
        console.print(
            "[yellow]Run `auto_cut_bot onboard --wizard` "
