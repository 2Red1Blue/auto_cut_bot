...
from auto_cut_bot.gateway.service import (
...
def create_gateway_app(
    *,
    ...
    gateway_app = typer.Typer(
        help="Start and manage the auto_cut_bot gateway.",
    ...
    def interactive_build_mode() -> BuildMode:
        # `auto_cut_bot gateway` is often launched by tests, supervisors, or service managers.
        # The higher-level `auto_cut_bot webui` command owns interactive first-run guidance.
        return "warn"
    ...
    def gateway(  # pyright: ignore[reportUnusedFunction]
        ...
        """Start the auto_cut_bot gateway."""
        ...
        if background:
            ...
            if (
                ...
                console.print(
                    ...
                    "`auto_cut_bot gateway --background`.[/dim]"
                )
            ...
            if (
                ...
                console.print(
                    "[dim]Stop it first, then rerun `auto_cut_bot gateway --background` "
            ...
            if result.message == "gateway_already_running":
                if result.promoted:
                    ...
                    console.print(
                        ...
                        "use `auto_cut_bot gateway stop` to stop it.[/dim]"
                    )
                else:
                    console.print(
    ...
    def gateway_restart(  # pyright: ignore[reportUnusedFunction]
        ...
        if result.message == "gateway_not_running":
            ...
            console.print(
                "[dim]Start a persistent gateway with `auto_cut_bot gateway --background`.[/dim]"
            )
            print_status(result.status)
            raise typer.Exit(1)
    ...
    def gateway_install_service(  # pyright: ignore[reportUnusedFunction]
        ...
        verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
        config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
        name: str = typer.Option("auto_cut_bot-gateway", "--name", help="Service name"),
