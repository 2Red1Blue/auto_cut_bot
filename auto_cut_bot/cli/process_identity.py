"""Give auto_cut_bot processes recognizable operating-system names."""
...
def set_cli_process_identity(args: list[str]) -> None:
    """Name this CLI process after the auto_cut_bot role it is running."""
    if os.name == "nt":
        # Windows process managers use the console launcher's executable name,
        # which packaging already generates as ``auto_cut_bot.exe``.
        return
    role = args[0] if args and args[0] in _ROLES else None
    _set_process_title(f"auto_cut_bot-{role}" if role else "auto_cut_bot")
