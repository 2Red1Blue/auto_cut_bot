...
from auto_cut_bot.cli.process_identity import named_executable
from auto_cut_bot.cli.runtime_config import _model_display
...
from auto_cut_bot.webui.session_identity import is_webui_session_key, webui_chat_id
...
_TUI_RELEASE_FILES = (
    ...
    "LGPL-2.0.txt",
    "LGPL-2.1.txt",
    "auto_cut_bot-tui-source.tar.gz",
)
_TUI_RELEASE_LIMITS = {
    ...
    "auto_cut_bot-tui-source.tar.gz": 20 * 1024 * 1024,
...
def launch_tui(
    ...
    *,
    ...
    try:
        env = os.environ.copy()
        env.pop("AUTO_CUT_BOT_TUI_WS_URL", None)
        env.pop("AUTO_CUT_BOT_TUI_API_TOKEN", None)
        env.update(
            {
                "AUTO_CUT_BOT_TUI_BOOTSTRAP_URL": f"{base_url}/webui/bootstrap",
                "AUTO_CUT_BOT_TUI_HEALTH_URL": _gateway_health_url(
                    config.gateway.host,
                    config.gateway.port,
                ),
                "AUTO_CUT_BOT_TUI_API_URL": base_url,
                "AUTO_CUT_BOT_TUI_MODEL": _model_display(config)[0],
                "AUTO_CUT_BOT_TUI_MODEL_PRESET": config.agents.defaults.model_preset or "default",
                "AUTO_CUT_BOT_TUI_WORKSPACE": str(tui_workspace),
                "AUTO_CUT_BOT_TUI_VERSION": __version__,
                "AUTO_CUT_BOT_TUI_ACCESS": (
                    "workspace access" if config.tools.restrict_to_workspace else "full access"
                ),
                "AUTO_CUT_BOT_TUI_THEME": theme,
                "AUTO_CUT_BOT_TUI_GATEWAY_STOP_COMMAND": _gateway_instance_command(
                    "stop",
        ...
        if bootstrap_secret:
            env["AUTO_CUT_BOT_TUI_BOOTSTRAP_SECRET"] = bootstrap_secret
        else:
            env.pop("AUTO_CUT_BOT_TUI_BOOTSTRAP_SECRET", None)
        if chat_id:
            env["AUTO_CUT_BOT_TUI_CHAT_ID"] = chat_id
        else:
            env.pop("AUTO_CUT_BOT_TUI_CHAT_ID", None)
        try:
...
def _resolve_tui_command() -> list[str]:
    override = os.environ.get("AUTO_CUT_BOT_TUI_BIN", "").strip()
    if override:
        executable = Path(override).expanduser().resolve(strict=False)
        if not executable.is_file():
            raise TuiUnavailableError(f"AUTO_CUT_BOT_TUI_BIN does not exist: {executable}")
    ...
    asset = f"auto_cut_bot-tui-{system}-{machine}{suffix}"
    ...
    if source_dir is not None:
        ...
        if not bun:
            raise TuiUnavailableError(
                ...
                "install Bun, then run `auto_cut_bot agent` again"
            )
    ...
    raise TuiUnavailableError(
        f"no native TUI archive is published for auto_cut_bot {__version__} on this platform; "
        ...
        "`auto_cut_bot agent --classic` if intentional"
    )
...
def _download_release_tui(asset: str) -> Path | None:
    ...
    if os.environ.get("AUTO_CUT_BOT_TUI_NO_DOWNLOAD") == "1":
        return None
    ...
    base = f"https://github.com/HKUDS/auto_cut_bot/releases/download/v{version}"
...
def _read_release_asset(url: str, *, max_bytes: int) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": f"auto_cut_bot/{__version__}"})
        ...
        def wait_for_ready(log_path: object) -> _GatewayHandle:
            ...
            if current.running:
                raise TuiUnavailableError(
                    ...
                    "Run `auto_cut_bot gateway status` and inspect logs at "
                    f"{log_path}; if it remains degraded, run `auto_cut_bot gateway restart`."
                )
            raise TuiUnavailableError(
        ...
        if status.running:
            if status.port not in {None, config.gateway.port}:
                raise TuiUnavailableError(
                    ...
                    "restart it or use `auto_cut_bot agent --classic`"
                )
            ...
            raise TuiUnavailableError(
                "the configured gateway port belongs to a different auto_cut_bot instance; "
                "stop that instance or use `auto_cut_bot agent --classic`"
            )
