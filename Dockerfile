...
RUN npm ci
COPY webui/ ./
RUN mkdir -p /app/auto_cut_bot/web && npm run build
...
# Keep the runtime environment writable by the non-root auto_cut_bot user. Enabled
...
ARG AUTO_CUT_BOT_EXTRAS=
COPY pyproject.toml README.md LICENSE THIRD_PARTY_NOTICES.md hatch_build.py ./
RUN mkdir -p auto_cut_bot && touch auto_cut_bot/__init__.py && \
    if [ -n "$AUTO_CUT_BOT_EXTRAS" ]; then \
        AUTO_CUT_BOT_SKIP_WEBUI_BUILD=1 uv pip install \
            --python "$VIRTUAL_ENV/bin/python" --no-cache ".[${AUTO_CUT_BOT_EXTRAS}]"; \
    else \
        AUTO_CUT_BOT_SKIP_WEBUI_BUILD=1 uv pip install \
...
COPY auto_cut_bot/ auto_cut_bot/
COPY scripts/install_channel_dependencies.py scripts/
COPY --from=webui-builder /app/auto_cut_bot/web/dist/ auto_cut_bot/web/dist/
RUN AUTO_CUT_BOT_SKIP_WEBUI_BUILD=1 uv pip install --python "$VIRTUAL_ENV/bin/python" --no-cache .
...
ARG AUTO_CUT_BOT_CHANNELS=whatsapp
RUN for channel in $(printf '%s' "$AUTO_CUT_BOT_CHANNELS" | tr ',' ' '); do \
...
# secrets through ${ANTHROPIC_API_KEY} / ${AUTO_CUT_BOT_WEB_TOKEN} env vars (resolved
...
RUN useradd -m -u 1000 -s /bin/bash auto_cut_bot && \
    mkdir -p /home/auto_cut_bot/.auto_cut_bot && \
    chown -R auto_cut_bot:auto_cut_bot /home/auto_cut_bot /app/.venv
...
# auto_cut_bot user via setpriv. The entrypoint drops privileges on every root start
# and fails closed if it cannot, so the agent never runs as root (see
# entrypoint.sh).
USER root
ENV HOME=/home/auto_cut_bot
