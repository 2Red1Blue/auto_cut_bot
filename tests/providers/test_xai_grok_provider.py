...
from auto_cut_bot.providers.oauth_model_catalog import OAuthModelCatalogSnapshot
from auto_cut_bot.providers.registry import ProviderModelSpec, find_by_name
from auto_cut_bot.providers.xai_grok_provider import (
...
def _mock_model_capabilities(
    ...
    *,
    ...
    monkeypatch.setattr(
        "auto_cut_bot.providers.xai_grok_provider.get_xai_grok_model_catalog",
        fake_catalog,
    )
...
async def test_provider_injects_hosted_x_search_and_required_proxy_headers(monkeypatch) -> None:
    ...
    monkeypatch.setattr("auto_cut_bot.providers.xai_grok_provider._request_xai", fake_request)
    ...
    assert headers["x-grok-client-identifier"] == "auto_cut_bot"
...
async def test_second_401_is_non_retryable_and_prompts_reauthentication(monkeypatch) -> None:
    ...
    async def always_unauthorized(*_args, **_kwargs):
        raise _XAIHTTPError(
            "xAI rejected the login. Sign in again with `auto_cut_bot provider login xai-grok`.",
            status_code=401,
            should_retry=False,
        )

    monkeypatch.setattr(
        "auto_cut_bot.providers.xai_grok_provider._request_xai",
        always_unauthorized,
    )
    provider = XAIGrokProvider()
    ...
    assert "auto_cut_bot provider login xai-grok" in (response.content or "")
...
async def test_raw_response_request_streams_text_usage_and_inline_citations(monkeypatch) -> None:
    ...
    monkeypatch.setattr("auto_cut_bot.providers.xai_grok_provider.httpx.AsyncClient", fake_client)
    deltas: list[str] = []
    ...
    events = [
        {
            ...
            "item_id": "x-search-1",
            "input": '{"query":"auto_cut_bot oauth"}',
        },
        {
    ...
    events = [
        {
            ...
            "item": {
                ...
                "id": "x-search-1",
                "status": "in_progress",
                "action": {"query": "auto_cut_bot oauth"},
            },
        },
        {
            "type": "response.output_item.done",
...
def test_client_version_rejection_explains_update_and_preserves_body() -> None:
    ...
    assert "xAI requires a newer Grok client version. Update auto_cut_bot and try again." in str(error)
