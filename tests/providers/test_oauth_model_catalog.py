...
from auto_cut_bot.providers.oauth_model_catalog import (
    OAuthModelCatalog,
    get_oauth_model_catalog,
    invalidate_oauth_model_catalog,
)
from auto_cut_bot.providers.openai_codex_provider import (
    DEFAULT_OPENAI_CODEX_MODELS_URL,
    OPENAI_CODEX_CATALOG_CLIENT_VERSION,
)
from auto_cut_bot.providers.registry import ProviderModelSpec
from auto_cut_bot.providers.xai_grok_provider import DEFAULT_XAI_GROK_MODELS_URL
from auto_cut_bot.providers.xai_oauth import XAIToken
...
def test_xai_catalog_fetches_remote_models_and_reuses_capability_metadata(
    ...
    monkeypatch.setattr(
        "auto_cut_bot.providers.xai_grok_provider.get_xai_oauth_storage_path",
        lambda: tmp_path / "auth" / "xai.json",
    )
    monkeypatch.setattr(
        "auto_cut_bot.providers.xai_grok_provider.get_xai_oauth_login_status",
        lambda: token,
    )
    monkeypatch.setattr(
        "auto_cut_bot.providers.xai_grok_provider.get_xai_oauth_token",
        lambda **_kwargs: token,
    )
    monkeypatch.setattr("auto_cut_bot.providers.xai_grok_provider.httpx.Client", fake_client)
...
def test_openai_codex_catalog_uses_account_catalog_and_filters_hidden_models(
    ...
    monkeypatch.setattr(
        "auto_cut_bot.providers.openai_codex_provider.FileTokenStorage",
        lambda **_kwargs: Storage(),
    )
    monkeypatch.setattr(
        "auto_cut_bot.providers.openai_codex_provider.get_codex_token",
        lambda **_kwargs: SimpleNamespace(access="secret", account_id="account-42"),
    )
    monkeypatch.setattr("auto_cut_bot.providers.openai_codex_provider.httpx.Client", fake_client)
...
def test_github_copilot_catalog_only_lists_compatible_chat_models(
    ...
    monkeypatch.setattr(
        "auto_cut_bot.providers.github_copilot_provider.get_storage",
        lambda: Storage(),
    )
    monkeypatch.setattr("auto_cut_bot.providers.github_copilot_provider.httpx.Client", fake_client)
