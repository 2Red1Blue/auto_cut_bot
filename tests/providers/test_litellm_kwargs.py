...
from auto_cut_bot.providers.base import ProviderCallContext
...
async def test_openai_compat_chat_stream_forwards_reasoning_deltas_deepseek_style() -> None:
    ...
    with patch("auto_cut_bot.providers.openai_compat_provider.AsyncOpenAI") as mock_openai:
...
async def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("auto_cut_bot.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
...
async def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("auto_cut_bot.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://auto_cut_bot.ai",
    ...
    headers = mock_client_cls.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://auto_cut_bot.ai"
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("AUTO_CUT_BOT_STREAM_IDLE_TIMEOUT_S", "0.01")
