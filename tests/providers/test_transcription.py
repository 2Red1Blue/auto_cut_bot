...
from auto_cut_bot.audio.transcription import (
...
from auto_cut_bot.providers.transcription import (
...
def test_transcription_registry_lists_providers_and_aliases() -> None:
    ...
    assert siliconflow.adapter == "auto_cut_bot.providers.transcription:OpenAITranscriptionProvider"
...
async def test_transcribe_audio_file_routes_openrouter_provider(audio_file: Path) -> None:
    ...
    with patch("auto_cut_bot.providers.transcription.OpenRouterTranscriptionProvider", StubOpenRouter):
...
async def test_transcribe_audio_file_routes_xiaomi_mimo_provider(audio_file: Path) -> None:
    ...
    with patch("auto_cut_bot.providers.transcription.XiaomiMiMoTranscriptionProvider", StubXiaomiMiMo):
        result = await transcribe_audio_file(audio_file, config)
...
async def test_transcribe_audio_file_routes_assemblyai_provider(audio_file: Path) -> None:
    ...
    with patch("auto_cut_bot.providers.transcription.AssemblyAITranscriptionProvider", StubAssemblyAI):
        result = await transcribe_audio_file(audio_file, config)
