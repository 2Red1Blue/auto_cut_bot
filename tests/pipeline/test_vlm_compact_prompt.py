...
def test_installed_semantic_authority_explicitly_selects_video_without_changing_default() -> None:
    ...
    assert authority.vlm_policy.prompt_version == VLM_CONTEXTUAL_VALIDATED_RECIPROCAL_CAUSAL_CORE_VIDEO_PROMPT_VERSION
...
def test_semantic_composition_preserves_installed_prompt_choice(tmp_path: Path) -> None:
    ...
    assert runtime.execution_profile.prompt_version == VLM_CONTEXTUAL_VALIDATED_RECIPROCAL_CAUSAL_CORE_VIDEO_PROMPT_VERSION
...
async def test_new_semantic_authority_does_not_relabel_a_v3_reconcile_context(monkeypatch: pytest.MonkeyPatch) -> None:
    assert load_installed_semantic_run_authority().vlm_policy.prompt_version == VLM_CONTEXTUAL_VALIDATED_RECIPROCAL_CAUSAL_CORE_VIDEO_PROMPT_VERSION
