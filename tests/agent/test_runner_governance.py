...
from auto_cut_bot.agent.context_governance import (
    BACKFILL_CONTENT,
    ContextGovernanceConfig,
    ContextGovernor,
    ContextWindowExceededError,
)
from auto_cut_bot.agent.runner import AgentRunner, AgentRunSpec
...
from auto_cut_bot.session.summary import SUMMARY_CONTINUATION_TEXT
...
async def test_runner_locally_fits_oversized_initial_transcript(monkeypatch):
    ...
    monkeypatch.setattr(
        "auto_cut_bot.agent.context_governance.estimate_prompt_tokens_chain",
        estimate,
    )
...
async def test_runner_drops_resumable_provider_state_when_request_is_fitted(monkeypatch):
    ...
    monkeypatch.setattr(
        "auto_cut_bot.agent.context_governance.estimate_message_tokens",
...
async def test_runner_counts_resumed_provider_state_before_dispatch(monkeypatch):
    ...
    monkeypatch.setattr(
        "auto_cut_bot.providers.conversation_state.estimate_prompt_tokens_chain",
        lambda *_args, **_kwargs: (100, "test-counter"),
    )
...
def test_snip_history_reserves_budget_for_tool_definitions(monkeypatch):
    ...
    monkeypatch.setattr("auto_cut_bot.agent.context_governance.estimate_prompt_tokens_chain", _estimate)
    token_sizes = {
        "system": 50,
        "old user": 200,
        "old assistant": 200,
