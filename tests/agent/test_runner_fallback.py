...
from auto_cut_bot.providers.openai_responses import resolve_compact_threshold
...
def test_fallback_models_accept_preset_refs_and_inline_configs() -> None:
    from auto_cut_bot.config.schema import Config, InlineFallbackConfig
...
def test_provider_snapshot_uses_smallest_fallback_context_window() -> None:
    from auto_cut_bot.config.schema import Config
    from auto_cut_bot.providers.factory import build_provider_snapshot
...
class TestRetryBeforeFailover:
    ...
    async def test_primary_recovers_before_fallback(self, retry_mode: str) -> None:
        ...
        with patch("auto_cut_bot.providers.base.asyncio.sleep", new_callable=AsyncMock):
    ...
    async def test_open_primary_circuit_remains_retryable_when_factory_fails(self) -> None:
        ...
        with (
            patch("auto_cut_bot.providers.fallback_provider.time.monotonic", return_value=100.0),
            patch("auto_cut_bot.providers.base.asyncio.sleep", new_callable=AsyncMock),
        ):
            result = await provider.chat_with_retry(
                [{"role": "user", "content": "hi"}],
                retry_mode="persistent",
