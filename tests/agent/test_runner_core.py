...
from auto_cut_bot.agent.context_governance import ContextWindowExceededError
...
def test_usage_or_estimate_replaces_reported_zero_for_content(monkeypatch) -> None:
    ...
    monkeypatch.setattr("auto_cut_bot.agent.runner.estimate_message_tokens", lambda message: 7)
...
def test_usage_or_estimate_counts_tool_call_output_for_reported_zero(monkeypatch) -> None:
    ...
    monkeypatch.setattr("auto_cut_bot.agent.runner.estimate_message_tokens", estimate_output)
    response = LLMResponse(
        content=None,
        tool_calls=[
            ToolCallRequest(
...
async def test_runner_applies_outer_wall_timeout_to_streaming_requests():
    ...
    with patch("auto_cut_bot.agent.runner.asyncio.wait_for", fake_wait_for):
        result = await runner.run(make_run_spec(provider,
