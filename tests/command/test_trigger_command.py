...
from auto_cut_bot.command.builtin import build_help_text, register_builtin_commands
...
async def test_trigger_command_creates_session_bound_local_trigger(tmp_path: Path) -> None:
    ...
    assert f"auto_cut_bot trigger {trigger.id} \"message\"" in response.content
