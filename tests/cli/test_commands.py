...
def _patch_webui_managed_gateway(
    ...
    from auto_cut_bot.gateway import GatewayStatus, RuntimeResult
...
def test_webui_background_points_to_the_single_persistent_gateway_command(
    ...
    assert "auto_cut_bot gateway --background --config" in compact_output
...
def test_webui_foreground_attaches_to_existing_managed_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "auto_cut_bot.cli.webui_support._gateway_health_ready",
...
def test_webui_foreground_refuses_occupied_webui_port(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "auto_cut_bot cannot start because one of its local ports is already in use" in compact_output
...
def test_webui_foreground_reports_an_existing_gateway_without_leaking_secret(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: False)
...
def test_gateway_agent_task_owns_initial_mcp_provider_close(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("auto_cut_bot.cli.gateway_runtime.MCPProvider", _FakeMCPProvider)
    monkeypatch.setattr("auto_cut_bot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("auto_cut_bot.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
