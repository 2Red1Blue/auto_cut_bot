...
from auto_cut_bot.gateway import GatewayRuntime, GatewayStartOptions, GatewayStatus, RuntimeResult
...
def test_status_reports_ready_provider_and_next_step(tmp_path) -> None:
    ...
    assert 'auto_cut_bot agent -m "Hello!"' in result.stdout
...
def test_status_reports_missing_key_for_anthropic_backends(
    ...
    assert 'Next: auto_cut_bot agent -m "Hello!"' not in output
...
def test_status_reports_missing_env_var_at_field(tmp_path, monkeypatch) -> None:
    name = "AUTO_CUT_BOT_TEST_STATUS_MISSING"
...
def test_agent_provider_setup_failure_points_to_shortest_routes(tmp_path) -> None:
    ...
    assert "Settings → Models" in output
    assert "auto_cut_bot onboard --wizard" in output
    assert "auto_cut_bot status --config" in output
...
def test_gateway_missing_provider_managed_start_for_webui_setup(
    ...
    monkeypatch.setattr(
        "auto_cut_bot.cli.webui_support.ensure_webui_bundle",
        lambda **_kwargs: None,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "auto_cut_bot webui --config",
        ),
        (
            ["gateway"],
            "Gateway configuration is invalid.",
            "auto_cut_bot gateway --config",
        ),
    ],
)
...
def test_status_missing_file_points_to_setup_without_changing_exit_contract(tmp_path) -> None:
    ...
    assert "auto_cut_bot webui" in result.stdout
    assert "auto_cut_bot onboard --wizard" in result.stdout
