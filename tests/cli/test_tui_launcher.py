...
import auto_cut_bot.cli.tui_launcher as tui_launcher
from auto_cut_bot.cli.agent import agent
from auto_cut_bot.cli.tui_launcher import (
    TuiSessionError,
    TuiUnavailableError,
...
def test_launcher_passes_the_canonical_model_preset_to_the_tui(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._resolve_tui_command", lambda: ["auto_cut_bot-tui"])
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._ensure_gateway", ensure_gateway)
    ...
    def popen(command: list[str], *, env: dict[str, str]) -> FakeProcess:
        assert command == ["auto_cut_bot-tui"]
        captured.update(env)
        events.append("spawned")
        return FakeProcess()

    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.subprocess.Popen", popen)
    ...
    assert result == 0
    assert captured["AUTO_CUT_BOT_TUI_MODEL"] == "openai/gpt-5.6"
    assert captured["AUTO_CUT_BOT_TUI_MODEL_PRESET"] == "Deep Research"
    assert captured["AUTO_CUT_BOT_TUI_WORKSPACE"] == str(Path.cwd().resolve())
    assert captured["AUTO_CUT_BOT_TUI_BOOTSTRAP_URL"] == (
        "http://127.0.0.1:8765/webui/bootstrap"
    )
    assert captured["AUTO_CUT_BOT_TUI_HEALTH_URL"] == "http://127.0.0.1:18790/health"
    assert captured["AUTO_CUT_BOT_TUI_BOOTSTRAP_SECRET"] == "bootstrap-secret"
    assert "AUTO_CUT_BOT_TUI_WS_URL" not in captured
    assert "AUTO_CUT_BOT_TUI_API_TOKEN" not in captured
    assert "AUTO_CUT_BOT_TUI_CHAT_ID" not in captured
    assert "AUTO_CUT_BOT_TUI_STATE_PATH" not in captured
...
def test_launcher_terminates_the_tui_when_gateway_start_fails(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._resolve_tui_command", lambda: ["auto_cut_bot-tui"])
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher.subprocess.Popen",
        lambda *args, **kwargs: FakeProcess(),
    )
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._ensure_gateway", fail_gateway)
...
def test_launcher_keeps_the_tui_alive_while_an_existing_gateway_recovers(
    ...
    monkeypatch.setattr("auto_cut_bot.gateway.GatewayRuntime", FakeRuntime)
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._resolve_tui_command", lambda: ["auto_cut_bot-tui"])
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher.subprocess.Popen",
        lambda *args, **kwargs: FakeProcess(),
    ...
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher._webui_endpoint_reachable",
...
def test_launcher_promotes_the_gateway_when_the_tui_detaches(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.subprocess.Popen", popen)
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher._ensure_gateway",
    ...
    assert captured["AUTO_CUT_BOT_TUI_GATEWAY_STOP_COMMAND"] == (
        f"auto_cut_bot gateway stop --config '{config_path}' --workspace '{workspace.resolve()}'"
    )
...
def test_explicit_tui_binary_must_exist(
    ...
    monkeypatch.setenv("AUTO_CUT_BOT_TUI_BIN", str(missing))
...
def test_windows_arm64_fails_instead_of_using_the_classic_prompt(
    ...
    monkeypatch.delenv("AUTO_CUT_BOT_TUI_BIN", raising=False)
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.platform.system", lambda: "Windows")
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.platform.machine", lambda: "ARM64")
...
def test_source_checkout_does_not_fall_back_to_a_release_tui_without_bun(
    ...
    monkeypatch.delenv("AUTO_CUT_BOT_TUI_BIN", raising=False)
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher._source_checkout_tui_dir",
        lambda: source_dir,
    )
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.shutil.which", lambda _name: None)
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher._download_release_tui",
...
def test_source_checkout_requires_project_and_tui_markers(
    ...
    module_path = project_root / "auto_cut_bot" / "cli" / "tui_launcher.py"
...
def test_interactive_agent_uses_native_tui(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.launch_tui", launch)
    monkeypatch.setattr("auto_cut_bot.config.loader.get_config_path", lambda: config_path)
    monkeypatch.setattr("auto_cut_bot.cli.agent.sys.stdin", SimpleNamespace(isatty=lambda: True))
    monkeypatch.setattr("auto_cut_bot.cli.agent.sys.stdout", SimpleNamespace(isatty=lambda: True))
...
def test_interactive_agent_does_not_silently_fall_back(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.agent._load_runtime_config", lambda *_args: config)
    monkeypatch.setattr("auto_cut_bot.cli.agent.console.print", lambda value: output.append(value))
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.launch_tui", unavailable)
    monkeypatch.setattr("auto_cut_bot.config.loader.get_config_path", lambda: tmp_path / "config.json")
    monkeypatch.setattr("auto_cut_bot.cli.agent.sys.stdin", SimpleNamespace(isatty=lambda: True))
    monkeypatch.setattr("auto_cut_bot.cli.agent.sys.stdout", SimpleNamespace(isatty=lambda: True))
    ...
    assert output == [
        ...
        "[dim]Use `auto_cut_bot agent --classic` only if you want the old prompt.[/dim]",
    ]
...
def test_default_agent_does_not_fall_back_outside_a_terminal(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.agent._load_runtime_config", lambda *_args: Config())
    monkeypatch.setattr("auto_cut_bot.cli.agent.sys.stdin", SimpleNamespace(isatty=lambda: False))
    monkeypatch.setattr("auto_cut_bot.cli.agent.sys.stdout", SimpleNamespace(isatty=lambda: True))
...
def test_source_checkout_refreshes_locked_tui_dependencies(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.subprocess.run", install)
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher.named_executable",
...
def test_source_checkout_fails_when_locked_dependencies_cannot_be_refreshed(
    ...
    source_dir.mkdir()
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher.subprocess.run",
...
def test_release_tui_is_verified_and_cached(
    ...
    asset = "auto_cut_bot-tui-linux-x64"
    binary = b"native-tui"
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.__version__", "9.9.9")
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._read_release_asset", read_asset)
...
def test_release_tui_rejects_bad_checksum(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher._read_release_asset",
...
def test_gateway_reuse_requires_the_matching_managed_instance(
    ...
    monkeypatch.setattr("auto_cut_bot.gateway.GatewayRuntime", FakeRuntime)
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._webui_endpoint_reachable", lambda _url: True)

    with pytest.raises(TuiUnavailableError, match="different auto_cut_bot instance"):
        _ensure_gateway(
            config,
            config_path=tmp_path / "config.json",
            workspace_override=str(workspace),
...
def test_gateway_reuses_the_matching_managed_instance(
    ...
    monkeypatch.setattr("auto_cut_bot.cli.tui_launcher._webui_endpoint_reachable", lambda _url: True)
    monkeypatch.setattr(
        "auto_cut_bot.cli.tui_launcher._gateway_health_ready",
...
def test_gateway_started_for_tui_stops_when_its_last_lease_exits(
    ...
    class FakeRuntime:
        ...
        def start_on_demand(self, options: object) -> SimpleNamespace:
            from auto_cut_bot.gateway import GatewayClientLease
