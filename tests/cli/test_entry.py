...
from auto_cut_bot.cli import entry
from auto_cut_bot.cli.entry import _agent_invocation_args, _native_tui_candidate
...
def test_root_shell_completion_keeps_root_subcommands() -> None:
    ...
    env.update(
        {
            "_AUTO_CUT_BOT_COMPLETE": "complete_bash",
            "COMP_WORDS": "auto_cut_bot ",
            "COMP_CWORD": "1",
        }
    )
    script = (
        "import sys; "
        "from auto_cut_bot.cli.entry import main; "
        "sys.argv = ['auto_cut_bot']; "
        "main()"
    )
...
def test_root_alias_dispatches_the_shared_agent_command(monkeypatch) -> None:
    ...
    monkeypatch.setattr(entry.sys, "argv", ["auto_cut_bot", "-m", "hello"])
    monkeypatch.setattr(
        entry,
