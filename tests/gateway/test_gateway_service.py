...
from auto_cut_bot.gateway import GatewayStartOptions
from auto_cut_bot.gateway.service import GatewayServiceInstaller, GatewayServiceOptions
...
def test_systemd_install_dry_run_renders_user_unit(tmp_path):
    ...
    result = installer.install(
        GatewayServiceOptions(
            start=GatewayStartOptions(
                port=18790,
                verbose=True,
                workspace="/tmp/auto_cut_bot workspace",
                config_path="/tmp/auto_cut_bot/config.json",
            ),
    ...
    assert result.path == tmp_path / ".config/systemd/user/auto_cut_bot-gateway.service"
    assert ("systemctl", "--user", "daemon-reload") in result.commands
    assert ("systemctl", "--user", "enable", "auto_cut_bot-gateway.service") in result.commands
    assert ("systemctl", "--user", "restart", "auto_cut_bot-gateway.service") in result.commands
    assert result.content is not None
    assert 'WorkingDirectory="/tmp/auto_cut_bot workspace"' in result.content
    assert 'ExecStart=/venv/bin/python -m auto_cut_bot gateway --foreground --port 18790 --verbose' in result.content
    assert '--workspace "/tmp/auto_cut_bot workspace" --config /tmp/auto_cut_bot/config.json' in result.content
...
def test_systemd_install_writes_unit_and_runs_commands(tmp_path):
    ...
    assert commands == [
        ...
        ["systemctl", "--user", "restart", "auto_cut_bot-gateway.service"],
    ]
...
def test_launchd_install_dry_run_renders_plist(tmp_path):
    ...
    result = installer.install(
        GatewayServiceOptions(
            start=GatewayStartOptions(
                port=18791,
                workspace="/Users/test/.auto_cut_bot/workspace",
                config_path="/Users/test/.auto_cut_bot/config.json",
            ),
    ...
    assert result.path == tmp_path / "Library/LaunchAgents/ai.auto_cut_bot.gateway.plist"
    assert result.content is not None
    payload = plistlib.loads(result.content.encode("utf-8"))
    assert payload["Label"] == "ai.auto_cut_bot.gateway"
    assert payload["ProgramArguments"] == [
        ...
        "18791",
        "--workspace",
        "/Users/test/.auto_cut_bot/workspace",
        "--config",
        "/Users/test/.auto_cut_bot/config.json",
    ]
...
def test_uninstall_systemd_removes_unit_and_reloads(tmp_path):
    ...
    unit = tmp_path / ".config/systemd/user/auto_cut_bot-gateway.service"
