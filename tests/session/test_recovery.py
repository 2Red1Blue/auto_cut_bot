...
from auto_cut_bot.bus.outbound_events import RecoveryStateEvent, SessionUpdatedEvent
...
from auto_cut_bot.webui import session_list_index, transcript
...
async def test_stale_incomplete_transcript_waits_for_confirmation(tmp_path: Path, monkeypatch) -> None:
    ...
    _persist(sessions, session)
    monkeypatch.setattr(
        "auto_cut_bot.webui.transcript.has_unfinished_transcript_tail",
        lambda _key: True,
    )
