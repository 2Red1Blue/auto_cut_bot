...
import auto_cut_bot.channels._setup as channel_setup_module
import auto_cut_bot.channels.registry as registry_module
...
from auto_cut_bot.channels.registry import channel_default_enabled, discover_plugins
...
def test_every_channel_is_a_self_contained_package() -> None:
    ...
    for name in EXPECTED_CHANNELS:
        ...
        assert plugin.runtime.startswith(f"auto_cut_bot.channels.{name}.runtime:")
...
def test_channel_manifests_only_import_contract_modules() -> None:
    channel_dir = Path(channel_setup_module.__file__).parent
    allowed_imports = {
        "auto_cut_bot.channels._manifest",
        "auto_cut_bot.channels.contracts",
        "auto_cut_bot.channels.plugin",
    }
    ...
    for name in EXPECTED_CHANNELS:
        ...
        allowed_channel_imports = {
            ...
            if module.startswith(f"auto_cut_bot.channels.{name}.")
...
def test_feishu_package_manifest_owns_runtime_and_webui_metadata() -> None:
    ...
    assert plugin is not None
    assert plugin.runtime == "auto_cut_bot.channels.feishu.runtime:FeishuChannel"
    assert plugin.dependencies == ("lark-oapi>=1.5.0,<2.0.0",)
    assert plugin.connector == "auto_cut_bot.channels.feishu.connect:FeishuConnectStore"
...
def test_weixin_package_manifest_owns_runtime_and_webui_metadata() -> None:
    ...
    assert plugin is not None
    assert plugin.runtime == "auto_cut_bot.channels.weixin.runtime:WeixinChannel"
    assert plugin.dependencies == ("qrcode[pil]>=8.0", "pycryptodome>=3.20.0")
    assert plugin.connector == "auto_cut_bot.channels.weixin.connect:WeixinConnectStore"
    assert plugin.webui == "webui/index.tsx"
...
for name in {sorted(EXPECTED_CHANNELS)!r}:
    plugin = load_channel_package(name)
    assert plugin is not None
    assert f"auto_cut_bot.channels.{{name}}.runtime" not in sys.modules
"""
