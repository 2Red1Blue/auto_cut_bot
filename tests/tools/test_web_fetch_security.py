...
from auto_cut_bot.agent.tools.web import WebFetchTool, _get_with_safe_redirects
from auto_cut_bot.config.schema import WebFetchConfig
from auto_cut_bot.security.network import PinnedDNSAsyncTransport
...
async def test_web_fetch_blocks_private_ip():
    tool = WebFetchTool()
    with patch("auto_cut_bot.security.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("auto_cut_bot.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_safe_redirect_requests_use_independent_pinned_dns_concurrently(monkeypatch):
    ...
    monkeypatch.setattr("auto_cut_bot.security.network.socket.getaddrinfo", _rebinding_resolver)
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    tool = WebFetchTool(
        ...
        user_agent="auto_cut_bot-test-agent",
    )
...
async def test_web_fetch_blocks_private_redirect_before_readability_request(monkeypatch):
    ...
    with patch("auto_cut_bot.security.network.socket.getaddrinfo", resolve_public_start_only):
...
async def test_web_fetch_blocks_private_redirect_before_returning_image(monkeypatch):
    ...
    monkeypatch.setattr("auto_cut_bot.agent.tools.web.httpx.AsyncClient", TransportAsyncClient)
    monkeypatch.setattr(web_module, "_pinned_dns_transport", lambda: object())
