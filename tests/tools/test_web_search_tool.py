...
from auto_cut_bot.agent.tools.web import WebSearchTool
from auto_cut_bot.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        assert kw["headers"]["User-Agent"] == "auto_cut_bot-search-test"
        return _response(json={
            "web": {"results": [{"title": "AutoCutBot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key", user_agent="auto_cut_bot-search-test")
    result = await tool.execute(query="auto_cut_bot", count=1)
    assert "AutoCutBot" in result
...
async def test_brave_search_retries_rate_limit_once(monkeypatch):
    ...
    monkeypatch.setattr("auto_cut_bot.agent.tools.web.asyncio.sleep", mock_sleep)
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
...
async def test_tavily_search(monkeypatch):
    ...
    tool = _tool(provider="tavily", api_key="tavily-key", user_agent="auto_cut_bot-search-test")
...
async def test_keenable_search(monkeypatch):
    async def mock_post(self, url, **kw):
        ...
        assert kw["headers"]["User-Agent"] == "auto_cut_bot-search-test"
        assert kw["headers"]["X-Keenable-Title"] == "auto_cut_bot"
        return _response(json={
            "results": [{"title": "Keen", "url": "https://keenable.ai", "description": "short", "snippet": "longer excerpt"}]
        })

    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="keenable", api_key="keen-key", user_agent="auto_cut_bot-search-test")
...
async def test_serper_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="serper", api_key="serper-key", user_agent="auto_cut_bot-search-test")
...
async def test_bocha_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="bocha", api_key="bocha-key", user_agent="auto_cut_bot-search-test")
...
async def test_volcengine_search(monkeypatch):
    async def mock_post(self, url, **kw):
        ...
        assert kw["headers"]["X-Traffic-Tag"] == "auto_cut_bot"
        assert kw["headers"]["User-Agent"] == "auto_cut_bot-search-test"
        assert kw["json"] == {
            "Query": "北京周边游",
            "SearchType": "web",
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="volcengine", api_key="volc-key", user_agent="auto_cut_bot-search-test")
...
async def test_searxng_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="searxng", base_url="https://searx.example", user_agent="auto_cut_bot-search-test")
    result = await tool.execute(query="test")
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("auto_cut_bot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import auto_cut_bot.agent.tools.web as web_mod
...
async def test_jina_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="jina", api_key="jina-key", user_agent="auto_cut_bot-search-test")
    result = await tool.execute(query="test")
...
async def test_kagi_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="kagi", api_key="kagi-key", user_agent="auto_cut_bot-search-test")
...
async def test_exa_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="exa", api_key="exa-key", user_agent="auto_cut_bot-search-test")
    result = await tool.execute(query="test", count=2)
...
async def test_olostep_package_missing_returns_install_hint(monkeypatch):
    ...
    assert result == (
        "Error: Olostep support is not installed. Run `auto_cut_bot plugins enable olostep`."
    )
