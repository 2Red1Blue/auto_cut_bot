...
from auto_cut_bot.agent.tools.web import WebFetchTool, _validate_url
...
def _patched_web_fetch():
    with (
        patch("auto_cut_bot.security.network.socket.getaddrinfo", _fake_resolve_public),
        patch("auto_cut_bot.agent.tools.web.httpx.AsyncClient", FakeClient),
        patch("auto_cut_bot.agent.tools.web._pinned_dns_transport", lambda: object()),
    ):
        yield
