...
main_source="https://github.com/HKUDS/auto_cut_bot/archive/refs/heads/main.zip"
...
install_failure_hint() {
  printf '%s\n' "Error: could not install auto_cut_bot from $install_source." >&2
  ...
  printf '  %s\n' "$python_bin -m venv ~/.auto_cut_bot/venv" >&2
  printf '  %s\n' "~/.auto_cut_bot/venv/bin/python -m pip install --upgrade $install_target" >&2
  printf '%s\n' "Then start setup with:" >&2
  printf '  %s\n' "auto_cut_bot onboard --wizard" >&2
  exit 1
}
...
run_auto_cut_bot() {
  case "$auto_cut_bot_runner" in
    uv)
      uv tool run --from "$install_target" auto_cut_bot "$@"
      ;;
    pipx)
      pipx run --spec "$install_target" auto_cut_bot "$@"
      ;;
    python)
      "$auto_cut_bot_python" -m auto_cut_bot "$@"
      ;;
    *)
...
is_fresh_auto_cut_bot_install() {
  [ -n "${HOME:-}" ] || return 1
  [ ! -e "$HOME/.auto_cut_bot/config.json" ]
}
...
install_with_active_python() {
  ...
  auto_cut_bot_runner="python"
  auto_cut_bot_python="$python_bin"
}

install_with_uv() {
  info "Installing or upgrading auto_cut_bot from $install_source with uv tool..."
  uv tool install --python "$python_bin" --force --upgrade "$install_target" || return 1
  auto_cut_bot_runner="uv"
}

install_with_pipx() {
  info "Installing or upgrading auto_cut_bot from $install_source with pipx..."
  pipx install --python "$python_bin" --force "$install_target" || return 1
  auto_cut_bot_runner="pipx"
}

write_managed_wrapper() {
  bin_dir="${AUTO_CUT_BOT_BIN_DIR:-$HOME/.local/bin}"
  wrapper="$bin_dir/auto_cut_bot"
  mkdir -p "$bin_dir" || return 0

  if [ -e "$wrapper" ] && ! grep -q "Generated by auto_cut_bot installer" "$wrapper" 2>/dev/null; then
  ...
  cat > "$wrapper" <<EOF
#!/bin/sh
# Generated by auto_cut_bot installer.
exec "$auto_cut_bot_python" -m auto_cut_bot "\$@"
EOF
  chmod +x "$wrapper" || return 0

  if ! command -v auto_cut_bot >/dev/null 2>&1; then
    info "Installed a auto_cut_bot launcher at $wrapper."
    info "Add $bin_dir to PATH to run auto_cut_bot directly."
  fi
}
...
install_with_managed_venv() {
  ...
  venv_dir="${AUTO_CUT_BOT_VENV:-$HOME/.auto_cut_bot/venv}"
  ...
  "$venv_python" - <<'PY' >/dev/null 2>&1 || fail "The managed venv uses Python older than 3.11. Remove it or set AUTO_CUT_BOT_VENV to a new path."
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY

  info "Installing or upgrading auto_cut_bot from $install_source in $venv_dir..."
  ensure_pip "$venv_python" || return 1
  "$venv_python" -m pip install --upgrade "$install_target" || return 1

  auto_cut_bot_runner="python"
  auto_cut_bot_python="$venv_python"
  write_managed_wrapper
}
...
if [ -n "$python_bin" ]; then
  ...
  "$python_bin" - <<'PY' >/dev/null 2>&1 || fail "auto_cut_bot requires Python 3.11 or newer"
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY
else
...
if [ "$dry_run" = "1" ]; then
  info "Dry run: would install or upgrade auto_cut_bot from $install_source."
  if python_is_virtual_env; then
    ...
    info "Dry run: would run auto_cut_bot as: $python_bin -m auto_cut_bot"
  elif command -v uv >/dev/null 2>&1; then
    info "Dry run: would run: uv tool install --python $python_bin --force --upgrade $install_target"
    info "Dry run: would run auto_cut_bot as: uv tool run --from $install_target auto_cut_bot"
  elif command -v pipx >/dev/null 2>&1; then
    info "Dry run: would run: pipx install --python $python_bin --force $install_target"
    info "Dry run: would run auto_cut_bot as: pipx run --spec $install_target auto_cut_bot"
  else
    venv_dir="${AUTO_CUT_BOT_VENV:-$HOME/.auto_cut_bot/venv}"
    ...
    info "Dry run: would run auto_cut_bot as: $venv_dir/bin/python -m auto_cut_bot"
  fi
  if [ "${AUTO_CUT_BOT_SKIP_WIZARD:-}" = "1" ]; then
    info "Dry run: would skip automatic setup because AUTO_CUT_BOT_SKIP_WIZARD=1."
  elif is_fresh_auto_cut_bot_install && has_browser_session; then
...
info "Installed auto_cut_bot:"
run_auto_cut_bot --version

if [ "${AUTO_CUT_BOT_SKIP_WIZARD:-}" = "1" ]; then
  info "Skipping automatic setup because AUTO_CUT_BOT_SKIP_WIZARD=1."
  info "Run this later: $(auto_cut_bot_try_command) webui"
  exit 0
fi

if is_fresh_auto_cut_bot_install && has_browser_session; then
  if run_auto_cut_bot webui --help >/dev/null 2>&1; then
    info "Starting auto_cut_bot WebUI..."
    info "Configure your first provider and model in Settings > Models."
    info "Run this later: $(auto_cut_bot_try_command) webui"
    run_auto_cut_bot webui --yes
    exit 0
  fi
  info "The installed release does not support auto_cut_bot webui yet."
  info "Falling back to the setup wizard..."
fi

if [ -t 0 ]; then
  info "Starting setup wizard..."
  run_auto_cut_bot onboard --wizard
elif : 2>/dev/null < /dev/tty; then
  info "Starting setup wizard..."
  run_auto_cut_bot onboard --wizard < /dev/tty
else
  info "Skipping setup wizard because no interactive terminal is available."
  info "Run this later: $(auto_cut_bot_try_command) onboard --wizard"
fi

info "Done. Try: $(auto_cut_bot_try_command) agent -m \"Hello!\""
