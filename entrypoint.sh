...
# re-exec as auto_cut_bot. Fail closed: if the privilege drop cannot be performed,
# exit rather than run the agent as root.
if [ "$(id -u)" = "0" ]; then
    chown -R auto_cut_bot:auto_cut_bot "$dir" 2>/dev/null || echo "[entrypoint] warning: chown $dir failed"
    if setpriv --reuid=auto_cut_bot --regid=auto_cut_bot --init-groups true 2>/dev/null; then
        echo "[entrypoint] dropping privileges to auto_cut_bot via setpriv"
        exec setpriv --reuid=auto_cut_bot --regid=auto_cut_bot --init-groups auto_cut_bot "$@"
    fi
...
Fix (pick one):
  Host:   sudo chown -R 1000:1000 ~/.auto_cut_bot
